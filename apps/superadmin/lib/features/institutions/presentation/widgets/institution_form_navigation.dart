          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    'Etapa ${current.index + 1} de ${InstitutionFormStep.values.length}',
...
final class _StepButton extends StatelessWidget {
  ...
  final InstitutionFormController controller;
  final InstitutionFormStep step;
  final bool compact;
  ...
  Widget build(BuildContext context) {
    ...
    final current = status == InstitutionFormStepStatus.current;
    final icon = switch (status) {
      InstitutionFormStepStatus.current => Icons.radio_button_checked_rounded,
      InstitutionFormStepStatus.complete => Icons.check_circle_outline_rounded,
      InstitutionFormStepStatus.error => Icons.error_outline_rounded,
      InstitutionFormStepStatus.incomplete => Icons.radio_button_unchecked_rounded,
    };
    final statusLabel = switch (status) {
      InstitutionFormStepStatus.current => 'selecionada',
      InstitutionFormStepStatus.complete => 'completa',
      InstitutionFormStepStatus.error => 'com erro',
      InstitutionFormStepStatus.incomplete => 'incompleta',
    };
    return Semantics(
      button: true,
      selected: current,
      ...
      child: Padding(
        ...
        child: TextButton.icon(
          key: status == InstitutionFormStepStatus.error
              ...
              TextButton.styleFrom(
                ...
                foregroundColor: WidgetStateProperty.resolveWith(
                  (states) =>
                      ...
                      ? colors.primary
                      : status == InstitutionFormStepStatus.error
                      ? colors.error
