...
///
/// The general map image, the markers and the per-location photos are not part
/// of this slice: their binaries belong to private R2 through the media
/// gateway, and no upload contract exists yet. The section says so rather than
/// showing a control that cannot store anything.
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: CoeloSpacing.space4),
        Text(
          'A planta geral, os marcadores e as fotos dos locais ainda não podem '
          'ser enviados: as imagens ficam no armazenamento privado e dependem do '
          'gateway de mídia.',
          key: const Key('locations-map-section-media-limit'),
          style: theme.textTheme.bodySmall,
        ),
        if (widget.onOpenCatalog != null && controller != null) ...[
          const SizedBox(height: CoeloSpacing.space4),
          OutlinedButton.icon(
