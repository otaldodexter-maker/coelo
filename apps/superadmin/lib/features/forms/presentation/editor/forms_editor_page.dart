...
final class _FormsEditorPageState extends State<FormsEditorPage> {
  ...
  Widget _questionTree(List<_EditorQuestionDraft> siblings, int index, {bool nested = false}) {
    ...
    return _QuestionCard(
      ...
      onRemoveOption: (option) {
        if (!current() || !question.options.contains(option) || _optionRemovalIssue(question, option) != null) return;
        _changeDraft(() {
  ...
  String? _optionRemovalIssue(_EditorQuestionDraft question, TextEditingController option) {
    ...
    if (_sections.any((section) => _flattenQuestions(section.questions).any((item) => item.loadedConditions.any(
...
final class _QuestionCardState extends State<_QuestionCard> {
  ...
  Widget _configuration() => Column(
    ...
    children: [
      ...
      if (widget.question.kind == FormItemKind.singleChoice || widget.question.kind == FormItemKind.multipleChoice) ...[
        const SizedBox(height: CoeloSpacing.space3),
        ...
        ReorderableListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Expanded(child: CoeloFormTextField(
                    controller: option,
                  ...
                  IconButton(
                    ...
                    onPressed: widget.optionRemovalIssue(option) != null ? null : () => widget.onRemoveOption(option),
                    icon: const Icon(Icons.delete_outline_rounded),
                  ),
                ]),
        ...
        OutlinedButton.icon(
          ...
          onPressed: widget.question.options.length < FormDefinitionLimits.maxOptionsPerItem ? widget.onAddOption : null,
          icon: const Icon(Icons.add_rounded),
