...
enum MealPlanRecurrenceKind { singleWeek, interval, daily, weekly, biweekly, cycleWeeks, specificDates }
...
final class MealPlanListFilter {
  const MealPlanListFilter({
    this.search, this.institutionId, this.unitId, this.classId, this.personId,
    this.periodStart, this.periodEnd, this.statuses = const {}, this.sources = const {},
    this.hasConflict, this.requiresReview, this.page = 0, this.pageSize = 12,
  }) : assert(page >= 0), assert(pageSize > 0 && pageSize <= 100);
  ...
  Map<String, Object?> toJson() => {
    'search': search, 'institutionId': institutionId, 'unitId': unitId,
    'classId': classId, 'personId': personId,
    'periodStart': periodStart?.toIso8601String(), 'periodEnd': periodEnd?.toIso8601String(),
    'statuses': statuses.map((v) => v.name).toList(), 'sources': sources.map((v) => v.name).toList(),
    'hasConflict': hasConflict, 'requiresReview': requiresReview, 'page': page, 'pageSize': pageSize,
  };
}

final class MealPlanPage {
  const MealPlanPage({required this.items, required this.total, required this.limit, required this.offset});
...
final class MealPlanRecurrence {
  MealPlanRecurrence({
    required this.kind, this.intervalWeeks, this.singleWeekStart, this.singleWeekEnd,
    this.cycleWeeks, this.weekdays = const {}, this.specificDates = const [],
  ...
  factory MealPlanRecurrence.fromJson(Map<String, Object?> j) => MealPlanRecurrence(
    ...
    intervalWeeks: _int(j['intervalWeeks']), singleWeekStart: _dateOrNull(j['singleWeekStart']),
    singleWeekEnd: _dateOrNull(j['singleWeekEnd']), cycleWeeks: _int(j['cycleWeeks']),
    weekdays: _list(j['weekdays']).map((v) => _int(v) ?? 1).toSet(),
    specificDates: _dateList(j['specificDates']), excludedDates: _dateList(j['excludedDates']),
  );
  ...
  Map<String, Object?> toJson() => {
    'kind': kind.name, 'intervalWeeks': intervalWeeks,
    'singleWeekStart': _dateValue(singleWeekStart), 'singleWeekEnd': _dateValue(singleWeekEnd),
    'cycleWeeks': cycleWeeks, 'weekdays': weekdays.toList()..sort(),
...
final class MealPlanMenuEntry {
  MealPlanMenuEntry({
    required this.mealType, this.dayContents = const ['', '', '', '', '', '', ''],
    this.customMealType, this.hasTime = false, this.startTime, this.endTime,
    this.dishName = '', this.details, this.hasNutrition = false, this.portionGrams,
    this.energyKcal, this.proteinG, this.carbohydrateG, this.fatG,
    this.restrictions = const [], this.image,
    this.scheduleKind = MealPlanMealScheduleKind.weekdays,
    this.weekdays = const {}, this.specificDates = const [],
  ...
  factory MealPlanMenuEntry.fromJson(Map<String, Object?> j) => MealPlanMenuEntry(
    ...
    customMealType: j['customMealType'] as String?, hasTime: _bool(j['hasTime']) ?? false,
    startTime: j['startTime'] as String?, endTime: j['endTime'] as String?,
    dishName: (j['dishName'] as String?) ?? '', details: j['details'] as String?,
    hasNutrition: _bool(j['hasNutrition']) ?? false, portionGrams: _double(j['portionGrams']),
    energyKcal: _double(j['energyKcal']), proteinG: _double(j['proteinG']),
    carbohydrateG: _double(j['carbohydrateG']), fatG: _double(j['fatG']),
    ...
    scheduleKind: _enumByName(
      MealPlanMealScheduleKind.values, j['scheduleKind'], MealPlanMealScheduleKind.weekdays),
    weekdays: _list(j['weekdays']).map((v) => _int(v) ?? 1).toSet(),
  ...
  Map<String, Object?> toJson() => {
    'mealType': mealType, 'dayContents': dayContents, 'customMealType': customMealType,
    'hasTime': hasTime, 'startTime': startTime, 'endTime': endTime,
    'dishName': dishName, 'details': details, 'hasNutrition': hasNutrition,
    'portionGrams': portionGrams, 'energyKcal': energyKcal, 'proteinG': proteinG,
    'carbohydrateG': carbohydrateG, 'fatG': fatG, 'restrictions': restrictions,
    'image': image?.toJson(), 'scheduleKind': scheduleKind.name,
    'weekdays': weekdays.toList()..sort(), 'specificDates': specificDates.map(_dateValue).toList(),
  };
}

final class MealPlanConflict {
  const MealPlanConflict({
    required this.conflictType, required this.scopeLevel, required this.scopeId,
    required this.dateRange, required this.mealType, required this.overlapWithIds,
    required this.requiredAction,
  });
  factory MealPlanConflict.fromJson(Map<String, Object?> j) => MealPlanConflict(
    conflictType: _enumByName(MealPlanConflictType.values, j['conflictType'], MealPlanConflictType.overlap),
    scopeLevel: (j['scopeLevel'] as String?) ?? 'global', scopeId: j['scopeId'] as String?,
    dateRange: (j['dateRange'] as String?) ?? '', mealType: (j['mealType'] as String?) ?? '',
    overlapWithIds: _list(j['overlapWithIds']).map((v) => v.toString()).toList(),
    requiredAction: _enumByName(
      MealPlanConflictAction.values, j['requiredAction'], MealPlanConflictAction.reject),
  );
...
final class MealPlan {
  const MealPlan({
    required this.id, required this.tenantId, this.institutionId, this.unitId,
    this.classId, this.personId, required this.name, required this.status,
    required this.sourceType, required this.scopeLevel, required this.scopeId,
    required this.startDate, required this.endDate, required this.recurrence,
    required this.menu, required this.allergens, required this.alerts,
    required this.attachments, required this.priority, required this.conflictState,
    required this.revision, required this.isDraft, required this.requiresReview,
    required this.createdBy, required this.updatedBy, this.inheritanceOriginId,
    this.planVariant = MealPlanPlanVariant.complete,
    this.audienceSegment = MealPlanAudienceSegment.students,
    this.visibilityMode = MealPlanVisibilityMode.immediate, this.visibleFrom,
    this.sourceTemplateId, this.sourceTemplateVersion, this.sourceTemplateName,
    this.scopeRules = const {}, this.simpleImage, this.simpleImageAlt,
    this.simpleNotes, this.isTemplate = false,
  });
  factory MealPlan.fromJson(Map<String, Object?> j) => MealPlan(
    ...
    sourceType: _enumByName(
      MealPlanSourceType.values, j['sourceType'] ?? j['source_type'], MealPlanSourceType.institution),
    scopeLevel: _enumByName(
      MealPlanScopeLevel.values, j['scopeLevel'] ?? j['scope_level'], MealPlanScopeLevel.institution),
    ...
    attachments: _list(j['attachmentsMeta'] ?? j['attachments_meta'])
    ...
    revision: _int(j['revision']) ?? 1, isDraft: _bool(j['isDraft'] ?? j['is_draft']) ?? true,
    ...
    inheritanceOriginId: j['inheritanceOriginId'] as String? ?? j['inheritance_origin_id'] as String?,
    planVariant: _enumByName(
      MealPlanPlanVariant.values, j['planVariant'] ?? j['plan_variant'], MealPlanPlanVariant.complete),
    audienceSegment: _enumByName(MealPlanAudienceSegment.values,
      j['audienceSegment'] ?? j['audience_segment'], MealPlanAudienceSegment.students),
    visibilityMode: _enumByName(MealPlanVisibilityMode.values,
      j['visibilityMode'] ?? j['visibility_mode'], MealPlanVisibilityMode.immediate),
    ...
    simpleImage: (j['simpleImage'] ?? j['simple_image_meta']) is Map
      ? MealPlanAttachmentMeta.fromJson(_map(j['simpleImage'] ?? j['simple_image_meta'])) : null,
  ...
  final String? institutionId, unitId, classId, personId, createdBy, updatedBy,
      inheritanceOriginId, sourceTemplateId, sourceTemplateName, simpleImageAlt, simpleNotes;
...
final class MealPlanDraft {
  const MealPlanDraft({
    this.requestId, this.mealPlanId, required this.tenantId, this.institutionId,
    this.unitId, this.classId, this.personId, required this.name,
    required this.sourceType, required this.scopeLevel, required this.scopeId,
    required this.startDate, required this.endDate, required this.recurrence,
    required this.menu, this.allergens = const [], this.alerts = const [],
    this.attachments = const [], required this.priority, required this.expectedRevision,
    this.inheritanceOriginId, this.planVariant = MealPlanPlanVariant.complete,
    this.audienceSegment = MealPlanAudienceSegment.students,
    this.visibilityMode = MealPlanVisibilityMode.immediate, this.visibleFrom,
    this.sourceTemplateId, this.sourceTemplateVersion, this.scopeRules = const {},
    this.simpleImage, this.simpleImageAlt, this.simpleNotes,
    this.saveAsTemplate = false, this.templateName,
  });
  final String? requestId, mealPlanId, institutionId, unitId, classId, personId,
      inheritanceOriginId, sourceTemplateId, simpleImageAlt, simpleNotes, templateName;
  ...
  Map<String, Object?> toJson() => {
    'tenantId': tenantId, 'institutionId': institutionId, 'unitId': unitId,
    'classId': classId, 'personId': personId, 'name': name,
    'sourceType': sourceType.name, 'scopeLevel': scopeLevel.name, 'scopeId': scopeId,
    'startDate': _dateValue(startDate), 'endDate': _dateValue(endDate),
    'recurrence': recurrence.toJson(), 'menu': menu.map((v) => v.toJson()).toList(),
    'allergens': allergens, 'alerts': alerts, 'attachments': attachments.map((v) => v.toJson()).toList(),
    'priority': priority, 'expectedRevision': expectedRevision,
    'inheritanceOriginId': inheritanceOriginId, 'planVariant': planVariant.name,
    'audienceSegment': audienceSegment.name, 'visibilityMode': visibilityMode.name,
    'visibleFrom': visibleFrom?.toIso8601String(), 'sourceTemplateId': sourceTemplateId,
    'sourceTemplateVersion': sourceTemplateVersion, 'scopeRules': scopeRules,
    'simpleImage': simpleImage?.toJson(), 'simpleImageAlt': simpleImageAlt,
    'simpleNotes': simpleNotes, 'saveAsTemplate': saveAsTemplate, 'templateName': templateName,
  };
}

final class MealPlanTemplate {
  const MealPlanTemplate({
    required this.id, required this.name, required this.planVariant,
    required this.audienceSegment, required this.status, required this.version,
    required this.payload, required this.createdAt, required this.updatedAt,
    this.tenantId, this.institutionId,
  });
  factory MealPlanTemplate.fromJson(Map<String, Object?> j) => MealPlanTemplate(
    id: j['id'].toString(), tenantId: j['tenant_id'] as String?,
    institutionId: j['institution_id'] as String?, name: (j['name'] as String?) ?? 'Sem nome',
    planVariant: _enumByName(
      MealPlanPlanVariant.values, j['plan_variant'], MealPlanPlanVariant.complete),
    audienceSegment: _enumByName(
      MealPlanAudienceSegment.values, j['audience_segment'], MealPlanAudienceSegment.students),
    status: (j['status'] as String?) ?? 'draft', version: _int(j['version']) ?? 1,
    payload: _map(j['payload']), createdAt: _date(j['created_at'] ?? DateTime.now()),
  ...
  MealPlan toDirectoryItem() => MealPlan(
    id: id, tenantId: tenantId ?? '', institutionId: institutionId, name: name,
    status: status == 'active' ? MealPlanStatus.published
      : status == 'archived' ? MealPlanStatus.archived : MealPlanStatus.draft,
    sourceType: MealPlanSourceType.global, scopeLevel: MealPlanScopeLevel.global,
    scopeId: '', startDate: createdAt, endDate: updatedAt,
    recurrence: MealPlanRecurrence(kind: MealPlanRecurrenceKind.singleWeek),
    menu: _menuList(payload['menu']), allergens: const [], alerts: const [],
    attachments: const [], priority: 0, conflictState: false, revision: version,
    isDraft: status == 'draft', requiresReview: false, createdBy: null, updatedBy: null,
    planVariant: planVariant, audienceSegment: audienceSegment,
    simpleImage: payload['simpleImage'] is Map
      ? MealPlanAttachmentMeta.fromJson(_map(payload['simpleImage'])) : null,
    simpleImageAlt: payload['simpleImageAlt'] as String?,
    simpleNotes: payload['simpleNotes'] as String?, isTemplate: true,
  );
}

final class MealPlanTemplateDraft {
  const MealPlanTemplateDraft({
    this.id, required this.name, required this.planVariant,
    required this.audienceSegment, required this.payload, this.expectedVersion = 0,
  });
  final String? id;
  final String name;
  final MealPlanPlanVariant planVariant;
  ...
  Map<String, Object?> toJson() => {
    'name': name, 'planVariant': planVariant.name,
    'audienceSegment': audienceSegment.name, 'payload': payload,
  };
}

final class MealPlanAudienceOption {
  const MealPlanAudienceOption({
    required this.id, required this.label, this.institutionId,
    this.unitId, this.groupId, this.audienceSegment,
  });
  factory MealPlanAudienceOption.fromJson(Map<String, Object?> j) => MealPlanAudienceOption(
    id: j['id'].toString(), label: (j['label'] as String?) ?? 'Sem nome',
    institutionId: j['institutionId'] as String? ?? j['institution_id'] as String?,
    unitId: j['unitId'] as String? ?? j['unit_id'] as String?,
    groupId: j['groupId'] as String? ?? j['group_id'] as String?,
    audienceSegment: j['audienceSegment'] == null ? null : _enumByName(
      MealPlanAudienceSegment.values, j['audienceSegment'], MealPlanAudienceSegment.students),
  );
...
final class MealPlanAudienceOptions {
  const MealPlanAudienceOptions({
    this.institutions = const [], this.units = const [], this.groups = const [],
    this.activities = const [], this.people = const [],
  });
  factory MealPlanAudienceOptions.fromJson(Map<String, Object?> j) => MealPlanAudienceOptions(
    institutions: _options(j['institutions']), units: _options(j['units']),
    groups: _options(j['groups']), activities: _options(j['activities']),
...
final class UnavailableMealPlanRepository implements MealPlanRepository {
  ...
  @override Future<MealPlanPage> fetchPage(MealPlanListFilter f) async => throw _fail();
  @override Future<MealPlanPage> fetchTemplatePage(MealPlanListFilter f) async => throw _fail();
  @override Future<MealPlan> getById(String id) async => throw _fail();
  @override Future<MealPlanTemplate> getTemplateById(String id) async => throw _fail();
  @override Future<MealPlanTemplate> saveTemplate(MealPlanTemplateDraft d, {required bool publish}) async => throw _fail();
  @override Future<MealPlanAudienceOptions> fetchAudienceOptions() async => throw _fail();
  @override Future<MealPlan> createOrUpdateDraft(MealPlanDraft d) async => throw _fail();
  @override Future<MealPlan> submitForReview(String id, String requestId, int rev) async => throw _fail();
  @override Future<MealPlan> publish(String id, String requestId, int rev) async => throw _fail();
  @override Future<List<MealPlanConflict>> checkConflicts({
    required String scopeLevel, required String scopeId, required DateTime startDate,
    required DateTime endDate, required MealPlanRecurrence recurrence,
    required List<MealPlanMenuEntry> menu,
  }) async => throw _fail();
  @override Future<MealPlan> fetchEffectiveSnapshot(MealPlanDraft d) async => throw _fail();
}
...
MealPlanStatus _status(String value) => switch (value) {
  ...
  'updated' => MealPlanStatus.updated, 'ended' => MealPlanStatus.ended,
  'archived' => MealPlanStatus.archived, _ => MealPlanStatus.draft,
};
...
String? _dateValue(DateTime? v) => v == null ? null
...
bool? _bool(Object? v) => v is bool ? v : v == null ? null : v.toString() == 'true';
