...
final class SupabaseMealPlanRepository implements MealPlanRepository {
  ...
  Future<MealPlanPage> fetchPage(MealPlanListFilter filter) async {
    ...
    return MealPlanPage(
      items: _plans(p['items']),
      total: _int(p['total']) ?? 0,
      limit: _int(p['limit']) ?? filter.pageSize,
      offset: _int(p['offset']) ?? filter.offset,
    );
  }
  ...
  Future<MealPlanPage> fetchTemplatePage(MealPlanListFilter filter) async {
    ...
    final items = _list(
      p['items'],
    ).map((v) => MealPlanTemplate.fromJson(_json(v)).toDirectoryItem()).toList();
    return MealPlanPage(
      items: items,
      total: _int(p['total']) ?? items.length,
      limit: _int(p['limit']) ?? filter.pageSize,
      offset: _int(p['offset']) ?? filter.offset,
    );
  }
  ...
  }) async => MealPlanTemplate.fromJson(
    _json(
      await _rpc('meal_plan_template_save', {
  ...
  @override
  Future<MealPlan> createOrUpdateDraft(MealPlanDraft draft) async => MealPlan.fromJson(
    _json(
      await _rpc('meal_plan_create_or_update_draft', {
        'p_request_id': draft.requestId ?? _requestId(),
        'p_payload': draft.toJson(),
        'p_meal_plan_id': draft.mealPlanId,
        'p_expected_revision': draft.expectedRevision,
      }),
    ),
  );
  @override
  Future<MealPlan> submitForReview(String id, String requestId, int revision) async =>
      MealPlan.fromJson(
        _json(
          await _rpc('meal_plan_submit_for_review', {
  ...
  @override
  Future<MealPlan> publish(String id, String requestId, int revision) async => MealPlan.fromJson(
    _json(
      await _rpc('meal_plan_publish', {
        'p_request_id': requestId,
        'p_meal_plan_id': id,
        'p_expected_revision': revision,
      }),
  ...
  Future<List<MealPlanConflict>> checkConflicts({
    ...
    final p = _json(
      await _rpc('meal_plan_conflicts_check', {
        'p_scope_level': scopeLevel,
        'p_scope_id': scopeId,
        'p_start_date': startDate.toIso8601String(),
        'p_end_date': endDate.toIso8601String(),
        'p_recurrence': recurrence.toJson(),
        'p_menu': menu.map((v) => v.toJson()).toList(),
      }),
    );
  ...
  @override
  Future<MealPlan> fetchEffectiveSnapshot(MealPlanDraft draft) async => MealPlan.fromJson(
    _json(await _rpc('meal_plan_effective_snapshot', {'p_payload': draft.toJson()})),
  );
  ...
  List<MealPlan> _plans(Object? v) => _list(v).map((x) => MealPlan.fromJson(_json(x))).toList();
  String _requestId() {
