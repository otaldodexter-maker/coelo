...
void main() {
  for (final width in const [375.0, 768.0, 1024.0, 1440.0]) {
    for (final brightness in Brightness.values) {
      testWidgets(
        'prototypes fit at ${width.toInt()} px in ${brightness.name}',
        (tester) async {
          ...
          await tester.pumpWidget(
            MaterialApp(
              ...
              home: DailyRoutineDirectoryPage(
                repository: DevelopmentRoutineRepository.content(),
                logout: unavailableSuperadminLogout,
              ),
            ),
          );
  ...
  testWidgets('supports keyboard, semantics, reduced motion and 200% text', (
    tester,
  ) async {
