...
void main() {
  testWidgets('ignores an old About read after controller and repository change', (tester) async {
    ...
    await tester.pumpWidget(_app(controller: controllerA, repository: repositoryA, activityId: 'a'));
    await tester.pump();

    await tester.pumpWidget(_app(controller: controllerB, repository: repositoryB, activityId: 'b'));
    await tester.pump();
    expect(repositoryB.requestCount, 1);

    repositoryB.complete(_page(institutionId: 'institution-b', activityId: 'b', description: 'Sobre B'));
    await tester.pump();
    expect(find.text('Sobre B'), findsOneWidget);

    repositoryA.complete(_page(institutionId: 'institution-a', activityId: 'a', description: 'Sobre A'));
    await tester.pump();
    expect(find.text('Sobre B'), findsOneWidget);
