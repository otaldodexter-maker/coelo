...
void main() {
  ...
  test('resgata ticket público pela Edge e rejeita URL não HTTPS', () async {
    ...
    const media = PrincipalNowMediaDescriptor(readTicket: 'ticket-active', mimeType: 'image/webp');
    ...
    secure = false;
    await expectLater(repository.resolveMedia(media), throwsA(isA<PrincipalNowFeedUnavailable>()));
  });
  ...
  test('mapeia negação da Edge para não autorizado', () async {
    ...
    await expectLater(
      SupabasePrincipalNowFeedRepository(client).resolveMedia(
        const PrincipalNowMediaDescriptor(readTicket: 'ticket-denied', mimeType: 'image/webp'),
      ),
      throwsA(isA<PrincipalNowFeedUnauthorized>()),
    );
  });
...
Map<String, dynamic> _row({
  ...
  'media': [
    {'read_ticket': 'ticket-$id', 'kind': 'media', 'mime_type': 'image/webp'},
  ],
};
