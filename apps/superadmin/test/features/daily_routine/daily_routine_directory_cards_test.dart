...
void main() {
  ...
  testWidgets('the status filter sends Arquivados to the server and clears back to null', (
    tester,
  ) async {
    final repo = repository();
    await pump(tester, repository: repo);
    expect(
      repo.pageQueries.last.status,
      isNull,
      reason: 'lista padrão: o servidor exclui arquivados',
    );

    tester
        .widget<CoeloAdminSingleSelectField<String>>(
          find.byKey(const Key('daily-routine-status-filter')),
        )
        .onChanged('archived');
    await tester.pumpAndSettle();
    expect(repo.pageQueries.last.status, 'archived');
    expect(repo.pageQueries.last.kind, RoutineEntryKind.model);
    ...
    tester
        ...
        .onChanged('Todos');
    await tester.pumpAndSettle();
    expect(repo.pageQueries.last.status, isNull);
  });
