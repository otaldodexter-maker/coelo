...
void main() {
  testWidgets('choice options add and remove preserve stable IDs through reload', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-opaque-b'))).onPressed, isNull);
    await tester.tap(save);
    await tester.pumpAndSettle();
    expect(reloaded.savedCommands.single.payload.sections.first.items.first.options.map((option) => option.id), sent.first.options.map((option) => option.id));
  ...
  testWidgets('choice options enforce fifty and recheck retained add at capacity', (tester) async {
    ...
    retained();
    await tester.pumpAndSettle();
    expect(tester.widget<ReorderableListView>(find.byKey(const ValueKey('forms-editor-options-parent'))).itemCount, 50);
  ...
  testWidgets('choice options protect complex references in another section', (tester) async {
    final api = _EditorApi(firstItems: [_optionsItem(3)], secondItems: [
      FormItem(id: 'remote-leaf', kind: FormItemKind.shortText, label: 'Remote leaf', position: 0,
        conditions: const [FormCondition.choice(sourceItemId: 'parent', optionIds: {'option-0', 'option-1'})]),
    ]);
    await tester.pumpWidget(_app(api, 'form-1'));
    await tester.pumpAndSettle();
    for (final id in ['option-0', 'option-1']) {
      expect(tester.widget<IconButton>(find.byKey(ValueKey('forms-remove-option-$id'))).onPressed, isNull);
    }
    expect(tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-option-2'))).onPressed, isNotNull);
    expect(find.text('Esta opção é usada em um ramo. Remova a referência antes de excluir.'), findsNWidgets(2));
  });

  for (final minimum in [2, 3]) {
    testWidgets('choice options preserve min $minimum max 50 without clamping', (tester) async {
      final api = _EditorApi(firstItems: [_optionsItem(3, config: FormItemConfig(minSelections: minimum, maxSelections: 50))]);
      await tester.pumpWidget(_app(api, 'form-1'));
      await tester.pumpAndSettle();
      final remove = tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-option-2')));
      if (minimum == 3) {
        expect(remove.onPressed, isNull);
        expect(find.text('O mínimo de seleções exige manter esta quantidade de opções.'), findsNWidgets(3));
      } else {
        remove.onPressed!();
        await tester.pumpAndSettle();
      }
  ...
  testWidgets('choice options removal clears only its transient branch selection', (tester) async {
    ...
    await tester.pumpWidget(_app(api, 'form-1'));
    await tester.pumpAndSettle();
    tester.widgetList<CoeloAdminToggleField>(find.byType(CoeloAdminToggleField))
        .singleWhere((field) => field.label == 'Desdobrar por resposta').onChanged!(true);
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-option-2'))).onPressed!();
    await tester.pumpAndSettle();
    expect(tester.widget<CoeloAdminSingleSelectField<String?>>(selector).value, isNull);
    expect(tester.widget<OutlinedButton>(find.widgetWithText(OutlinedButton, 'Adicionar pergunta ao ramo')).onPressed, isNull);
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('choice options retained callbacks ignore a replacement editor', (tester) async {
    ...
    await tester.pumpAndSettle();
    final add = tester.widget<OutlinedButton>(find.byKey(const ValueKey('forms-add-option-parent'))).onPressed!;
    final remove = tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-option-0'))).onPressed!;
    ...
    await tester.pumpAndSettle();
    expect(tester.widget<ReorderableListView>(find.byKey(const ValueKey('forms-editor-options-parent'))).itemCount, 4);
  ...
  testWidgets('choice options retained delete cannot remove a different option', (tester) async {
    ...
    await tester.pumpWidget(_app(api, 'form-1'));
    await tester.pumpAndSettle();
    final retained = tester.widget<IconButton>(find.byKey(const ValueKey('forms-remove-option-option-0'))).onPressed!;
    retained();
    await tester.pumpAndSettle();
    retained();
    await tester.pumpAndSettle();
    ...
    final options = tester.widget<ReorderableListView>(find.byKey(const ValueKey('forms-editor-options-parent')));
...
FormItem _optionsItem(int count, {FormItemConfig config = const FormItemConfig()}) => FormItem(
  id: 'parent', kind: FormItemKind.multipleChoice, label: 'Choices', position: 0, config: config,
  options: [for (var i = 0; i < count; i++) FormOption(id: 'option-$i', label: 'Option $i', position: i)],
);
