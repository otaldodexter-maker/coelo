...
void main() {
  ...
  testWidgets('protects production and exposes the development unit directory', (tester) async {
    ...
    expect(find.byKey(const Key('superadmin-navigation-units')), findsOneWidget);
    ...
    expect(find.byKey(const Key('superadmin-onboarding-tour')), findsOneWidget);
  ...
  testWidgets('opens unit creation and editing from the development directory', (tester) async {
    ...
    router.go(SuperadminRoutes.devUnits);
    await tester.pumpAndSettle();
    final firstUnitCard = find.byWidgetPredicate((widget) {
      final key = widget.key;
      return key is ValueKey<String> &&
          key.value.startsWith('unit-card-') &&
          key.value != 'unit-card-grid' &&
          !key.value.startsWith('unit-card-surface-');
    }).first;
