...
void main() {
  ...
  testWidgets('real /principal-profile reads circulars through the Principal projection', (
    ...
    // Hosted inside the Superadmin shell, "Circulares" also names a host menu
    // entry, so the tab must be addressed inside the Principal page itself.
    final circularsTab = find.descendant(
      of: find.byType(PrincipalProfileRoutePage),
      matching: find.text('Circulares'),
    );
    await tester.ensureVisible(circularsTab);
    await tester.tap(circularsTab);
    await tester.pumpAndSettle();
