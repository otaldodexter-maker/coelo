...
Importação e exportação reais não fazem parte do backend Supabase, da integração
ou do E2E do MVP atual. Os botões permanecem visíveis nas telas aplicáveis para
preservar a composição e a descoberta do produto.
...
`deferred-post-mvp` e não bloqueiam a conclusão do MVP.
...
tratavam parser, upload, download, jobs ou exportação real como requisito do MVP.
