...
Instituições é a baseline obrigatória para toolbar, ações de arquivo, criação, tabela, cards e paginação. Criar/Editar Instituição é a baseline dos formulários e de seus rodapés. Comunicações segue a composição responsiva do anexo 6; Publicar/Editar Circular segue o compositor e a prévia do anexo 9 sem importar componentes do pacote Principal para o Superadmin.
