"""Agora API smoke for the nominal R08 synthetic fixture; natural 24-hour retention.
...
def main():
    ...
    try:
        ...
        def media(action, payload, actor=token):
            return api("/functions/v1/now-media", {"action": action, **payload}, actor)
        ...
        rpc("list_visible_now_publications", scope)
        draft = rpc("save_now_draft", {"p_request_id": str(uuid.uuid4()), "p_draft": {"institution_id": args.institution, "unit_id": args.unit, "group_id": args.group, "caption": "R08 G4 Agora synthetic private PNG API proof", "overlay_text": "", "crop_scale": 1, "crop_x": 0, "crop_y": 0, "cover_position": 0, "audiences": ["school_staff"], "publish_at": None}, "p_publication_id": None, "p_expected_version": 0})
        publication_id, version = draft["id"], draft["version"]
        manifest["resources"]["publication_id"] = publication_id
        save()
        envelope = {"request_id": str(uuid.uuid4()), "institution_id": args.institution, "publication_id": publication_id, "name": manifest["fixture"]["name"], "mime_type": "image/png", "size_bytes": len(data), "unit_id": args.unit, "group_id": args.group, "kind": "media", "duration_seconds": None, "rights_confirmed": False}
        ...
        publication = rpc("publish_now", {"p_request_id": str(uuid.uuid4()), "p_publication_id": publication_id, "p_expected_version": version, "p_publish_at": None})
        published = True
        manifest["expires_at"] = publication["expires_at"]
        manifest["publication_expiry_verified"] = False
        save()
        rows = rpc("list_visible_now_publications", scope)
        ...
        if publication_id:
            manifest["cleanup"] = "synthetic Agora retained until natural 24-hour expiry; private master preserved by existing retention" if published else "synthetic draft retained for inspection; no deletion API invoked"
