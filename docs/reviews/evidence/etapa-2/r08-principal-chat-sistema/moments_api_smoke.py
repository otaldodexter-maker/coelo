"""Momentos API smoke for the nominal R08 synthetic fixture.
...
def main():
    ...
    try:
        ...
        def media(action, payload, actor=token):
            return api("/functions/v1/moments-media", {"action": action, **payload}, actor)

        scope = {"p_institution_id": args.institution, "p_unit_id": args.unit, "p_group_id": args.group, "p_limit": 50, "p_cursor": None}
        # This read proves authorization before creating any resource.
        rpc("list_visible_moments", scope)
        draft = rpc("save_moments_draft", {"p_request_id": str(uuid.uuid4()), "p_draft": {"institution_id": args.institution, "unit_id": args.unit, "group_id": args.group, "caption": "R08 G4 Momentos synthetic private PNG API proof", "audiences": ["school_staff"]}, "p_publication_id": None, "p_expected_version": 0})
        publication_id, version = draft["id"], draft["version"]
        manifest["resources"]["publication_id"] = publication_id
        save()
        envelope = {"request_id": str(uuid.uuid4()), "institution_id": args.institution, "publication_id": publication_id, "name": manifest["fixture"]["name"], "mime_type": "image/png", "size_bytes": len(data), "duration_milliseconds": None, "display_order": 0}
        status, prepared = media("prepare", envelope)
        check("prepare_r2", status == 200 and isinstance(prepared.get("upload_url"), str), status)
        ...
        status, finalized = media("finalize", {**envelope, "asset_id": asset_id, "finalize_request_id": str(uuid.uuid4())})
        check("finalize_binding", status == 200 and finalized.get("asset_id") == asset_id, status)
        rpc("publish_moment", {"p_request_id": str(uuid.uuid4()), "p_publication_id": publication_id, "p_expected_version": version})
        published = True
        rows = rpc("list_visible_moments", scope)
        row = next((r for r in rows if r.get("publication_id") == publication_id), None)
        check("published_readback", row is not None and len(row["media"]) == 1)
        ticket = row["media"][0]["asset_id"]
        status, _ = media("read", {"asset_id": ticket}, actor=None)
        check("anonymous_denied", status in (401, 403), status)
        status, _ = media("read", {"asset_id": str(uuid.uuid4())})
        check("unknown_asset_denied", status == 403, status)
        if negative:
            status, _ = media("read", {"asset_id": ticket}, actor=negative)
            check("other_actor_asset_denied", status in (401, 403), status)
        status, resolved = media("read", {"asset_id": ticket})
        check("authorized_read", status == 200 and resolved.get("mime_type") == "image/png", status)
        ttl = resolved["expires_in"]
        check("bounded_read_ttl", isinstance(ttl, int) and 0 < ttl <= 120)
        ...
        rows = rpc("list_visible_moments", scope)
        check("fresh_server_reload", any(r.get("publication_id") == publication_id for r in rows))
        ...
        if token and publication_id:
            try:
                if published:
                    rows = rpc("list_visible_moments", scope)
                    row = next((r for r in rows if r.get("publication_id") == publication_id), None)
                    rpc("withdraw_moment", {"p_request_id": str(uuid.uuid4()), "p_publication_id": publication_id, "p_expected_version": None, "p_reason": "R08 G4 synthetic API proof cleanup"})
                    rows = rpc("list_visible_moments", scope)
                    check("withdrawal_readback_absent", not any(r.get("publication_id") == publication_id for r in rows))
                    manifest["cleanup"] = "publication withdrawn; private master follows existing retention"
                    status, _ = media("read", {"asset_id": asset_id})
                    check("withdrawn_asset_read_denied", status == 403, status)
                else:
                    manifest["cleanup"] = "synthetic draft retained for inspection; no deletion API invoked"
            except Exception as error:
                manifest["status"] = "failed"
