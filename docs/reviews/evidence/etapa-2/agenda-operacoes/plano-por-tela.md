...
| Agenda | Criar/editar/lifecycle / `agenda.create`, `agenda.edit` | 5–6/6 somente cache commit `e86f275` | Cliente; `public.superadmin_agenda_save/command` simuladas | root + `activities_contract_read` | Escrita tardia não repopula após negação; leitura não ressuscita excluído | Validação de recorrência; persistência real e concorrência backend |
