...
| Agenda | Cliente dedicado READ039 | 3/6 local isolado | Três RPCs v2 simuladas; DTO parcial sem personIds; nenhuma rota/DI | root + `activities_contract_read` | 24 RED iniciais; review encontrou 1 RED de lembrete vazio; 49 finais PASS; analyzer limpo | GREEN SQL, composição read-only reservada, lifecycle, HTTP real e reload |
| Agenda | Controller READ039 e fronteira de sessão | 3/6 local isolado | Estado de página/contextos/detalhe; nenhum BD | root + `activities_contract_read` | 12 RED iniciais; review de negativa tardia/reentrância; 20 finais PASS e analyzer limpo | View parcial em execução; composição deve fornecer revisão real da fronteira; sem HTTP/DI |
...
| Cardápios/modelos | Origem paginada e versão histórica | 6/6 local `a5ad81d` + cobertura de seleção | Estado do wizard; nenhum BD | root + review read-only | 4RED originais; 60regressões PASS; depois 3 controles de trocar/limpar/manter e 36wizard PASS | FK/autorização e salvamento/reload reais; refresh de detalhe na troca não exercitado |
...
| Cardápios/modelos | Conteúdo plano/histórico e edição | 6/6 correção cliente nominal | Parser/serializer/wizard; SQL somente lido | root + `activities_contract_read` | Matriz aprovada; 15 RED dados + 2 RED wizard; 100 regressões PASS | Preservação opaca primeiro nível; backend legado/039, persistência real e R2 abertos |
