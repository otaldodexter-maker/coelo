...
Leitura das constraints produtivas confirmou que os checks atuais da Turma R05, do link de atividade ativo e dos campos de Turma avaliam como vÃ¡lidos. `23514` sem mensagem nao identifica o constraint de origem. Para manter a prÃ³xima prova QA segura, o cÃ³digo agora transforma somente nomes conhecidos do erro em `CHECK_GROUP_HANDLE`, `CHECK_GROUP_TYPE`, `CHECK_ACTIVITY_DATES`, `CHECK_ROLE_SCOPE`, `CHECK_AUDIT` ou `CHECK_RECEIPT`; o fallback Ã© `CHECK_UNKNOWN`. Nenhum texto do banco sai do app. A prova pgTAP histÃ³rica de group save no espelho continua incompatÃ­vel com o baseline em quatro expectativas de handles de outra frente; nÃ£o foi alterada. Teste focal do repositÃ³rio 10 PASS e analyze focal PASS.
