...
- **Item 1 (cliente)** — P47 = A (`e689033ff`): o assistente de Cardápios não falha fechado por tenant vazio; o servidor valida (`meal_plan_scope_allowed`). **Rota real**: `meal-plans.model-create` e `model-edit` criam/editam e persistem (modelo `d132698c…` published v1 → v2, reload carrega do servidor; capturas 02–08). `meal-plans.create/edit/publish`: o assistente abre e chega a *Enviar e publicar*, mas `meal_plan_create_or_update_draft` caía com `cannot extract elements from an object` porque o cliente enviava `scopeRules` como mapa e a RPC itera `jsonb_array_elements`; corrigido no cliente (lista de regras por escopo + leitura tolerante) com teste de contrato; a prova pela tela após o rebuild não coube no tempo.
...
- **meal-plans.create/edit/publish** — repetir a prova pela tela com o build que contém a correção de `scopeRules`.
