...
$cliVersion = (& npx.cmd --yes supabase@2.116.0 --version 2>$null | Out-String).Trim()
if ($LASTEXITCODE -ne 0 -or $cliVersion -cne '2.116.0') { throw 'Pinned CLI unavailable' }
...
function Invoke-FixtureSql([string]$Sql) {
  $output = $Sql | & docker exec -i $container psql -X -U postgres -d postgres -v ON_ERROR_STOP=1 -At 2>&1
  if ($LASTEXITCODE -ne 0) { throw 'Local fixture SQL failed; raw output withheld' }
...
$created = $false
try {
  # Require a CLI-initialized empty ledger. This harness does not create/repair it.
  $ready = Invoke-FixtureSql "select count(*)=0 from supabase_migrations.schema_migrations;"
  if ($ready -cne 't') { throw 'Dedicated empty ledger required' }
  $absent = Invoke-FixtureSql "select to_regnamespace('d01_transport_probe') is null;"
  if ($absent -cne 't') { throw 'Probe namespace collision' }
  $null = Invoke-FixtureSql @'
...
'@, [Text.UTF8Encoding]::new($false))
  $failedOutput = & npx.cmd --yes supabase@2.116.0 --agent no db push --local --skip-vault --workdir $root --yes 2>&1
  if ($LASTEXITCODE -eq 0 -or (($failedOutput | Out-String) -notmatch 'D01_FORCED_LEDGER_FAILURE')) {
    throw 'Expected exact ledger injection failure was not witnessed'
  }
  $rolledBack = Invoke-FixtureSql "select to_regclass('d01_transport_probe.committed_together') is null and not exists(select 1 from supabase_migrations.schema_migrations where version='$version');"
  if ($rolledBack -cne 't') { throw 'DDL/ledger rollback atomicity FAIL' }
  Write-Output 'CLI_LEDGER_FAILURE_ROLLBACK_PASS'
  $null = Invoke-FixtureSql 'drop trigger d01_transport_probe_reject on supabase_migrations.schema_migrations;'
  $controlOutput = & npx.cmd --yes supabase@2.116.0 --agent no db push --local --skip-vault --workdir $root --yes 2>&1
  if ($LASTEXITCODE -ne 0) { throw 'CLI success control failed; raw output withheld' }
  $committed = Invoke-FixtureSql "select to_regclass('d01_transport_probe.committed_together') is not null and (select count(*)=1 from supabase_migrations.schema_migrations where version='$version' and name='d01_atomicity_fixture' and cardinality(statements)=4);"
  ...
  if ($created) {
    $null = Invoke-FixtureSql 'drop trigger if exists d01_transport_probe_reject on supabase_migrations.schema_migrations; drop function if exists d01_transport_probe.reject_ledger();'
  }
}
