...
def main():
    ...
    output = here / "save-diagnostic-proof.json"
    if output.exists():
    ...
    try:
        status, body = transport.call(base, headers, "/rest/v1/rpc/superadmin_group_directory", {
            "p_search": "Turma R09 Estrutura 1634", "p_institution_ids": ["190dd028-3125-452d-8502-612bfa1029de"],
        ...
        status, body = transport.call(base, headers, "/rest/v1/rpc/superadmin_people_identity_lookup_v1", {
            "p_kind": "name", "p_query": "QA R04 Pessoa", "p_institution_id": "190dd028-3125-452d-8502-612bfa1029de", "p_unit_id": "f5284f2f-b487-4100-bc0b-ffbcbb7d3db3"})
        result["lookupHttp"] = status
        result["candidateShape"] = [{k: v for k,v in row.items() if k in ("person_id", "id", "status", "access", "resolution_access")} for row in (body.get("data", []) if isinstance(body, dict) else [])]
        result["checks"]["lookupReadable"] = status == 200 and body.get("ok") is True
    finally:
