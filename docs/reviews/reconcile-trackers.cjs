...
const actions=front.map(f=>{
  ...
  return {id,family,screen:f[1],scope,done,fe,be,evidence:o.evidence||`archive/2026-09-08/${names[0]} (buscar ${id}); archive/2026-09-08/${names[1]} (buscar ${id})`,historicalFrontend:unquote(f[4]),historicalBackend:unquote(b[4]),frontendStatus:'pending-verification',backendStatus:scope==='flutter-only'?'not-applicable':scope==='deferred-post-mvp'?scope:scope==='gate-formal-mvp'?scope:'pending-verification',integratedStatus:scope==='mvp'?'pending-verification':scope};
});
...
fs.writeFileSync(`${dir}/inventario-etapa-2.json`,JSON.stringify({sourceCommit:base,generatedAt:'2026-09-08',counts,actions},null,2)+'\n');
