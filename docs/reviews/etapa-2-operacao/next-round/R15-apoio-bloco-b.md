...
### AP-2 — membership de responsável para o Principal abrir (achado D1 da coordenadora) — candidato pronto, FECHADO SEM APLICAÇÃO (OQ-048)
...
- Nenhuma pergunta pendente. A questão do AP-2 (Principal para responsável sem membership) foi
  resolvida pela coordenadora como lacuna de contrato → OQ-048 (spec 064/Etapa 3); o candidato
  `20260917113000` fica versionado sem aplicação.

## Estado dos pedidos

| AP | Fatia do B | Recebido | Entrega | Estado |
|---|---|---|---|---|
| AP-2 | 7 — `agora.publish` Famílias (leitura da responsável recusada por `now_actor`) | pedido antecipado da coordenadora (~15:1x BRT) e registro formal do B no handoff (`b6b58093d`, 18:36 UTC), encerrado pela coordenadora às 18:45 UTC | candidato: migration `20260917113000_qa_r15_guardian_membership_v1` + pgTAP 16/16 (não aplicado) | **não aplicado** — reaberto às 15:4x (B: `agora.publish` 403 `now_permission_denied`) e **parado na checagem prévia**: com a membership `guardian` a responsável ainda recebe `now_permission_denied` (`has_context_permission` só lê papéis/overrides da membership; falta grant `now.publications.read` — decisão de contrato, OQ-048). Candidato `3a9ee569d` versionado em `dev` |
