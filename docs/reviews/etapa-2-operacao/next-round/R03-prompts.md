...
| --- | --- | --- |
| D1 | Perfil Principal: referência com Acompanhar/Seguidores ou composição atual (Mensagem + 3 métricas)? | Composição atual; Coelo não é rede social |
| D2 | Stream no Agora desde o início (US$ 5/mês ao promover o primeiro vídeo) ou zero absoluto, tocando MP4 do R2? | Zero absoluto até métrica |
...
| D4 | Circulares: Agendar (campo inline como as irmãs), Encerrar e Excluir ligados no MVP? | Sim, os três, campo inline |
...
| D7 | Lançamentos (`daily-routine.publish`): construir tela mínima ou adiar | Adiar; comando continua no backend |
| D8 | Guarda de saída do editor de Rotina pela barra lateral (move 6 goldens) | Ligar, igual a Instituições |
| D9 | Sobre do Perfil: criar capacidade de leitura `profiles.about.read` para membros | Sim |
| D10 | Baseline visual de `errors.409` | Família das páginas de erro existentes |
