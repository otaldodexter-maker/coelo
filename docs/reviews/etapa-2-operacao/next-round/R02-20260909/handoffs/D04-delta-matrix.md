...
Base56eb3f19, revisão1 de14:55 BRT. App único: apps/superadmin.
...
| people.create | Formulário: nome legal opcional comprovado | Write interno aprovado e identity repository |
| people.edit | Formulário: nome legal opcional comprovado | Write interno e persistência/recarga real |
...
| access-profiles.list | Catálogo: sem certificação nova | Decisão sobre visibilidade global institucional e ACL/helpers |
...
| invites.list | Debounce invalida retorno anterior | Runtime do diretório com persona qualificada |
...
| invites.resend | Comando existente; nenhuma entrega real alegada | Composição normal e envio nominal autorizado |
| invites.revoke | Comando existente; nenhuma mutação remota | Composição normal, revogação e recarga reais |
| child-safety.list | Candidato held-golden adc902eaf: epochs/count e layout sem crash | Golden pendente e realm interno |
| child-safety.child | Candidato held-golden adc902eaf: contexto ausente não usa unidade alheia | Realm interno e runtime autorizado |
...
| internal-users.create | Falhas tratadas; contrato de convite indisponível preservado | Contrato nominal de criação/convite |
| internal-users.edit | Negação limpa editor; erro recuperável permite retry | Rota normal de escrita bloqueada |
