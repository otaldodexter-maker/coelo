...
**FE 199/199 (100%), BE 186/186 (100%), E2E 186/186 (100%)**, Owner 44/53 (83,02%) — 18/09, Sessão RESERVA:
owner.r12-04, owner.r12-05, owner.r12-06, owner.r12-08 e owner.r12-49 `done` na rota real (eram 39/53 em 17/09);
...
terminais no MVP** (meta FE/BE/E2E da R16 atingida), 9 Owner items abertos/parciais (14 em 17/09), 19 resíduos H, 2 itens da ADR
...
3. **Contrato e dívida técnica R16**: `oq048-membership` (onboarding cria membership `guardian`), ~~`recipients-bug`~~ (concluído 18/09, lote 82), ~~`can-remove`~~ (concluído 18/09: feed segue a RPC, D6, lote 82), `momentos-ux`, ~~`celular-mascara`~~ (concluído 18/09, D5, lote 82), `orfaos-cardapio`, `form-diario`, `forms-v2-qa`, `espelho-cli`, `h02-aal2`, ~~`testes-vermelhos`~~ (concluído 18/09, D7: 21 testes funcionais corrigidos — 19 de referência velha no teste, 2 regressões reais no código; goldens ficam na Etapa 3; `R16-handoff-reserva.md`).
...
|---|---|---|---|---|
| owner.r12-04 | daily-routine.list, attendance.dashboard | done / verified / done / verified-e2e | docs/reviews/evidence/etapa-2/r16-reserva/attendance-history-snapshot-20260918.md; docs/reviews/evidence/etapa-2/r14-sessao-10/attendance-history-20260916.md; specs/052-superadmin-attendance-history-and-routine-snapshot.md | Concluído 18/09 (Sessão RESERVA, rota real 3014 com `qa-r06-operacoes`): `20260916180000_attendance_call_history_v1` está em produção desde o lote 74 (16/09); Histórico `/attendance/history` com lista (8 chamadas), filtros (Situação Em andamento 3 / Concluídas 6), cursor por PostgREST (páginas de 3, `has_more`), abre o detalhe, reload; negativas: responsável (`qa-r15-responsavel`) → 403 42501, detalhe alheio → null, status inválido → 22023. Decidido em 18/09 (Owner delegou à RESERVA): "Publicar lançamento" fica no Histórico › Lançamentos de rotina (spec 052 §3). |
| owner.r12-05 | attendance.create | done / verified / done / verified-e2e | docs/reviews/evidence/etapa-2/r16-reserva/attendance-activity-negatives-and-mass-20260918.md; docs/reviews/evidence/etapa-2/r15-bloco-b/attendance-contexto-atividade-20260917.md; docs/reviews/evidence/etapa-2/r14-sessao-1/attendance-create-20260915.md | Concluído 18/09 (Sessão RESERVA): negativas do contexto Atividade em produção por PostgREST com `qa-r06-operacoes` — atividade inexistente e atividade inelegível (vinculada a outra turma) → 403 42501 "activity outside scope"; instituição alheia declarada → 403 42501 "context outside scope"; nenhuma chamada criada. `participants []` explicado pelo contrato (esperados derivam de `activity_group_participants`; atividade sem participantes é válida — Mesa R16, ADR 0044). |
| owner.r12-06 | attendance.create, daily-routine.apply | done / verified / done / verified-e2e | docs/reviews/evidence/etapa-2/r16-reserva/attendance-history-snapshot-20260918.md; docs/reviews/evidence/etapa-2/r14-sessao-10/attendance-routine-snapshot-20260916.md; specs/052-superadmin-attendance-history-and-routine-snapshot.md | Concluído 18/09 (Sessão RESERVA, rota real): `20260916183000_attendance_routine_snapshot_v1` em produção desde o lote 74; rotina `8b317b01` ativada pela tela (escopo instituição QA R04); chamada `98f6796c` (3 participantes) concluída → detalhe e Histórico mostram "registrada na conclusão" (snapshot revisão 2, 14:07:30Z), reload; reabrir (v5) e concluir (v6) preservam o mesmo snapshot; legado `cd60f2d8` mostra "rotina atual (não registrada na época)"; versão defasada → 409 PT409. |
| owner.r12-08 | attendance.mark, attendance.correct, attendance.finish, daily-routine.apply | done / verified / done / verified-e2e | docs/reviews/evidence/etapa-2/r16-reserva/attendance-activity-negatives-and-mass-20260918.md; docs/reviews/evidence/etapa-2/r16-reserva/attendance-history-snapshot-20260918.md; docs/reviews/evidence/etapa-2/r14-sessao-1/attendance-create-20260915.md | Concluído 18/09 (Sessão RESERVA, D4): fixture `20260918123000` executada no lote 82 (Turma QA R06 Transferencia com "QA R15 Crianca 3/4", vínculos ativos); rota real com duas turmas (3 e 2 alunos) e rotina vinculada `8b317b01`: mark (individual e em lote), finish, reabrir/finish, correct após conclusão (v5, motivo, reload). |
...
| owner.r12-33 | medication.create, medication.edit, medication.detail | partial / verified / done / verified-e2e (responsável) | docs/reviews/evidence/etapa-2/r16-reserva/medication-bell-v2-20260918.md; docs/reviews/evidence/etapa-2/r14-sessao-10/medication-notifications-20260916.md; specs/053-superadmin-medication-in-app-notifications.md | 18/09 (Sessão RESERVA): v1 em produção desde o lote 74; **v2 `20260918120000` aplicada no lote 82** (E7 = b: responsável destinatária; `recipients-bug` corrigido nos dois leitores); pgTAP 27/27 + 29/29 no espelho fiel. Rota real: plano `2ecf267e` (QA R15 Crianca 1) editado (v2, dose 7) e dose registrada pela tela; `qa-r15-responsavel` vê `plan.created`, `plan.updated` e `dose.recorded` no sino (PostgREST, OQ-048). **Resíduo:** admin da unidade / educador da turma não observáveis em produção — as 11 memberships do tenant QA R04 Cuidado são pessoas de serviço (nunca destinatárias); exigiria fixture de membership (SQL fora da autorização a–d). Decisão do Owner: aceitar a prova do espelho ou autorizar a fixture. |
...
| owner.r12-46 | account.profile | deferred / FE verified na rota real (Bloco A 17/09): foto PNG real pelo seletor nativo, avatar do cabeçalho atualizado só após confirmação do servidor, remover foto, sigla QR, celular inválido bloqueado × válido gravado, layout A+ "Meu acesso" na mesma linha com rolagem interna. / BE done: Edge account-media (prepare/finalize/read/remove) + save_v2/get em produção; asset alheio/removido → denied (422). / E2E verified-e2e: reload e nova sessão veem a foto do R2; remoção relida. | docs/reviews/evidence/etapa-2/r15-bloco-a/account-profile-20260917.md | **Celular concluído em 18/09 (D5, Sessão RESERVA, lote 82 item b):** E.164 `+55DDD9NNNNNNNN` no servidor (`normalize_mobile_phone_e164`, 22023 `invalid_account_mobile_phone`) e máscara `+55 (DD) 9NNNN-NNNN` no cliente; provado na rota real (evidência r16-reserva/account-mobile-phone-e164-20260918.md). Resta o layout A+ "Meu acesso" (Etapa 3, tela a tela) e, opcional, mapear denied da Edge para 403. 8 goldens da Conta divergem só no campo Celular (não regravados, D7). |
| owner.r12-49 | assessments.entry, assessments.gradebook, assessments.detail, assessments.close, assessments.reopen | done / verified / done / verified-e2e | docs/reviews/evidence/etapa-2/r16-reserva/assessments-close-reopen-20260918.md; docs/reviews/evidence/etapa-2/r14-sessao-3/assessments-close-reopen/assessments-close-reopen-20260915.md; docs/reviews/evidence/etapa-2/r13-coordenacao/lotes-63-64-provas-producao-20260914.md | Concluído 18/09 (Sessão RESERVA, rota real 3014, aba visível autenticada como `qa-r06-operacoes`): diário `d2c945d8` devolvido (v12) → reenviado (v13) → revisado (v14), cada passo relido após reload; versão defasada → 409 `SAI_CONCURRENT_CHANGE` sem mutação. Sem candidato SQL: as RPCs estão em produção desde `20260910180350`. Nada duplicado. |
...
| Goldens pré-existentes | R12–R15 | 55 regravadas (30 na R14, 25 Principal na R15, E4); restam falhas fora do E4: notice_directory (filtro Estado), forms (editor/operations/response), invites (golden + responsive), meal_plans (diretório), platform_users | Regravar só com diff decidido por família (Etapa 3, D7). `invite_responsive_test` corrigido em 18/09. 18/09: 8 goldens de `account_pages_golden_test` regravadas por decisão do Owner (profile ×4: máscara do Celular, D5; settings ×4: só o avatar do cabeçalho, E4), suíte verde. |
| Testes pré-existentes vermelhos | anterior à R14 | **Corrigidos em 18/09 (D7, Sessão RESERVA)**: 21 funcionais (invite_responsive ×9, model_save_completion ×3, principal_real_route, principal_profile_for_you, activity_routes, location_consumer_routes, production_circular_attachments, forms_directory_internal_reader, principal_moments_publication, directory_composition, superadmin_circular_pages); 2 eram regressão de código (Histórico: nomes fora da convenção do composto; `CoeloCreateAction` estourava a 375/200%) | Censo antes/depois em `R16-handoff-reserva.md`; restam só goldens (Etapa 3). |
...
| `owner.r12-46` layout A+ "Meu acesso" e máscara do Celular | R13/R15 | Card A+ pendente; **Celular concluído em 18/09 (E.164 + máscara, lote 82)** | Layout A+ na Etapa 3; r12-46 permanece deferred. |
