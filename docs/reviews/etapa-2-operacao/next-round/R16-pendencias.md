...
| owner.r12-33 | medication.create, medication.edit, medication.detail | partial / FE local-green (Sessão 10, 16/09): sino já lia `context_notification_*`; rótulos para plano criado/atualizado/status e dose registrada (resumo por desfecho). / BE local-green: `20260916190000_medication_in_app_notifications_v1` (destinatários = admins da unidade + educadores da turma, sem responsáveis; editar plano → `medication.plan.updated`; cada dose → `medication.dose.recorded`; `not_tracked` silencia) + pgTAP 29/29 no espelho; criar plano já notificava em produção (trigger existente, audiência mantida); **aplicada em produção no lote 74 (16/09 17:40 UTC; correção de 18/09 — o texto anterior dizia "NÃO aplicada")**. / Pendente (Sessão RESERVA 18/09): v2 `20260918120000_medication_in_app_notifications_v2` (E7 = b: responsável destinatário; `recipients-bug`: membership `guardian`/`student` não é equipe) pgTAP 27/27 + v1 29/29 no espelho `mirror-r16-reserva`; aplicar no lote 82 (item a) e provar o sino com admin da unidade, educador da turma e `qa-r15-responsavel` por PostgREST. | docs/reviews/evidence/etapa-2/r14-sessao-10/medication-notifications-20260916.md; specs/053-superadmin-medication-in-app-notifications.md | Lote 82 (item a, autorização nominal de 18/09): aplicar a v2 pelo rito; editar plano e registrar dose reais; observar o sino com três identidades (admin da unidade, educador da turma, `qa-r15-responsavel` por PostgREST — OQ-048); sem e-mail/push. |
