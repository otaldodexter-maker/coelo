...
- `forms.location-answer` — **não concluído** (massa/versão + ambiente no fim da sessão). Fatos apurados: a pessoa-ator de `qa-r06-formularios` (`9f944691-6a32-450d-b65d-b8bfb7dd07d9`, via `superadmin_internal_actor_people`) **é participante elegível** da ocorrência de hoje `7256b047…` (participation `2e372091…`; leitura D1 da coordenadora) e `form_get_occurrence_for_response` devolve `can_edit true` para ela — porém a ocorrência (e as 30 diárias geradas hoje) usa `form_version_id 49c338d7…`, a versão **publicada antiga**, que só tem a pergunta de texto curto; a pergunta Local ("Em qual local?") existe apenas na versão de trabalho (management_version 4) ainda **não publicada** (`form_publish` é o que promove `working_version_id → published_version_id`). Caminho que falta: "Publicar ou agendar → Publicar agora" no editor de `4555ba07…`, depois gerar ocorrência nova (novo horário/agendamento) e responder pelo deep link `/forms/4555ba07…/occurrences/<nova>/respond` (localização, persistência, reload, inválido, alheio → 403). A conta da responsável `qa-r15-responsavel` não serve para esta rota (o shell só admite identidades internas; sem fallback Principal). No fim da sessão o Chrome/SwiftShader travou duas vezes no login (comandos do driver sem resposta), o que impediu executar a publicação dentro do prazo do Owner (16:20).
- (auth resolvido por decisão E10.)

## Sobra para a R16

- `forms.location-answer` (única ação não terminal do Bloco A): publicar a v4 do form `4555ba07…`, gerar ocorrência nova e responder como `qa-r06-formularios` (roteiro acima; ~30 min com ambiente estável).
- Resíduos sem action_id listados acima (tooltip empilhado em Perfis; chip "Mensagens" sobre o rodapé; rótulos padrão do editor de formulários; Celular sem máscara; `superadmin_forms_*_v2` negando a identidade QA; `form_get_editor`/`superadmin_access_profile_detail` com id inexistente → 500 P0002; Edge `account-media` 422 em vez de 403).
- Massa residual em produção (sem senha): perfil `281699f2…` renomeado "R15 A Perfil QA renomeado" (v2, 2 capacidades); usuário interno `bf6008f0…` v4 com perfil Owner (restaurado); conta `qa-r06-acessos` com sigla "QR", celular `+55 11 98888-0001`, sem foto (asset `e2a00029…` removido); formulário `90b905a1…` "R15 A Formulario QA" (draft v4, 2 perguntas, asset `9e437fa9…` expirado/deleted); formulário `4555ba07…` com agendamento trocado para **Diário** (30 ocorrências novas 12/09–11/10 na versão antiga; considerar voltar para "Uma vez" pela tela); pedido de recuperação de senha enviado à caixa do Owner (17:43Z).
- Goldens: nenhum regravado nesta sessão (4 goldens do diretório de usuários internos seguem na deriva do cabeçalho, ADR 0041 C1).
