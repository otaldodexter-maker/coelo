...
- `forms.location-answer` — massa: a conta `qa-r15-responsavel` existe (AP-1 rodou 18:10Z, pessoa `da915f98…` ativa com `person_auth_links`), mas (a) o Superadmin nega o login a contas não internas (`superadmin_auth_bootstrap_context` → 403 `SAI_INTERNAL_CONTEXT_DENIED`, tela "Não foi possível entrar"; captura forms-location-01) e (b) `list_my_principal_contexts` devolve `[]` para ela — sem contexto no Principal hospedado. Ou seja, não há porta de entrada para responder pela tela até a massa dar à responsável um contexto no Principal (vínculo responsável↔instituição/criança visível ao `list_my_principal_contexts`) e ela constar na audiência/ocorrência. Preparado: form `4555ba07…` v4 com pergunta Local ("[R04-QA] Local Sala Azul"), ocorrência `open` de hoje `7256b047…` (e 30 outras diárias até 11/10, geradas pela troca de frequência); quando a conta tiver contexto, incluir a pessoa na audiência (Turma `368a5cea…` ou "Toda a instituição" após reconcile) e responder pelo deep link `/forms/4555ba07…/occurrences/<id>/respond` (localização, persistência, reload, inválido rejeitado, alheio → 403).
