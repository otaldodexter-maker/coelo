...
- `forms.location-answer` — massa: a conta `qa-r15-responsavel` existe (AP-1 rodou 18:10Z, pessoa `da915f98…` ativa com `person_auth_links`), mas (a) o Superadmin nega o login a contas não internas (`superadmin_auth_bootstrap_context` → 403 `SAI_INTERNAL_CONTEXT_DENIED`, tela "Não foi possível entrar"; captura forms-location-01) e (b) `list_my_principal_contexts` devolve `[]` para ela — sem contexto no Principal hospedado. Confirmado no código (`superadmin_auth_scope.dart` ~342–370 + `supabase_superadmin_auth_context_gateway.dart`): após o login o shell exige `superadmin_auth_bootstrap_context` com `platform.read`; se negado, faz `signOut` — **não há fallback para conta só de responsável**, e as features do Principal hospedado não têm rota de resposta de formulário. Caminho viável pela tela: identidade interna (`qa-r06-formularios`) com pessoa ativa em QA R04 vinculada por `person_auth_links` (fixture no padrão da AP-1, pedido ao B′ como AP-2) e incluída na audiência; a coordenadora está alinhando com B/B′. Preparado: form `4555ba07…` v4 com pergunta Local ("[R04-QA] Local Sala Azul"), ocorrência `open` de hoje `7256b047…` (e 30 outras diárias até 11/10, geradas pela troca de frequência); quando a conta tiver contexto, incluir a pessoa na audiência (Turma `368a5cea…` ou "Toda a instituição" após reconcile) e responder pelo deep link `/forms/4555ba07…/occurrences/<id>/respond` (localização, persistência, reload, inválido rejeitado, alheio → 403).
