...
$manifest=Get-Content -LiteralPath (Join-Path (Split-Path -Parent $proposal) 'manifest-after.json') -Raw | ConvertFrom-Json
...
try {
  ...
  & git -C $isolated init --quiet
  if ($LASTEXITCODE) {throw 'ISOLATED_GIT_INIT_FAILED'}
  & git -C $isolated apply (Join-Path $proposal 'wrappers.patch')
  if ($LASTEXITCODE) {throw 'PROPOSAL_PATCH_FAILED'}
  ...
  $checks.Add('three proposed PowerShell files parse')
  ...
  Assert-Rejected {& $resolver -TargetVersion '20260908051500'} 'requires target 20260909190000'
  ...
  $guardPath=Join-Path $isolated 'packages/coelo_database/migrations/20260909190000_d04_child_safety_internal_reads.sql'
...
[IO.File]::WriteAllText((Join-Path $proposal 'dry-run.json'),($result | ConvertTo-Json -Depth 5)+"`n",[Text.UTF8Encoding]::new($false))
$result
