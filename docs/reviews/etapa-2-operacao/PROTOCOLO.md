...
FE concluído cobre todos os critérios próprios do cliente; BE concluído cobre todos os provedores e critérios próprios do backend. E2E requer o fluxo pela UI normal com backend real, persistência/reload e negativas. Tentativa de integração real pode aumentar o exame sem aumentar a conclusão. Incorporação de código em Git é outro estado, registrado por SHA, sem percentual funcional fabricado.

Mostrar numerador, denominador, data, IDs/fontes e faltas por tela/ação. Exame total e conclusão ativa possuem denominadores explicitamente distintos: separar adiadas, gates e N/A. Não somar percentuais de camadas. Manter tabela agregada por família e detalhe das219ações nos três rastreadores oficiais; os relatórios derivados apontam as fontes e não substituem os rastreadores.
