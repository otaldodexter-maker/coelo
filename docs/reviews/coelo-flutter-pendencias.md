...
| 5.1 | Turmas / Diretório | `groups.list` | `OPERATIONS-WAVE`: diretório Flutter revalidado; unauthorized omite controles, failure preserva Retry + criação autorizada e limpar no-results sincroniza campo/query. Produção continua read-only/fail-closed para mutações. | `local-green` | B | I | A | C | Avançada | 1.5 h executada; backend/remoto/E2E residual | Cards/tabela 375–1440 light/dark, 200%, estados, filtros, paginação e interações verdes; 34 goldens reais inspecionados no pacote de Turmas. |
| 5.2 | Criar Turma | `groups.create` | `OPERATIONS-WAVE`: formulário local `/dev` usa baseline Criar Instituição, toggles Coelo e golden real; rota produtiva permanece bloqueada sem capability autoritativa. | `local-green` | B | I | A | C | Avançada | 1.5 h executada; escrita produtiva bloqueada | Criar/validar/salvar local, 375–1440, 200% e rodapé responsivo provados; não declarar CRUD produtivo. |
| 5.3 | Turmas / Detalhe-editar | `groups.edit` | `OPERATIONS-WAVE`: golden inválido de edição foi corrigido para abrir a Turma real; hierarquia bloqueada, dirty state e formulário responsivo revalidados. Produção permanece fail-closed. | `local-green` | B | I | A | C | Avançada | 1.5 h executada; detail/reload integrado residual | Abrir/editar/cancelar local e deep link `/dev` verdes; rota produtiva de mutação bloqueada sem tocar backend. |
| 5.4 | Turmas / Membros | `groups.members` | `OPERATIONS-WAVE`: pessoas, profissionais e convites agora persistem no repositório fake entre save/reload e ganharam golden próprio; produção segue sem contrato/gateway e não usa RPC legado. | `local-green` | B | I | A | C | Avançada | 1.5 h executada local; contrato/backend/remoto/E2E bloqueados | `/dev` preserva bindings em reload e cobre subetapa responsiva; negar/conflito/tenant/teclado integrado dependem de spec e backend próprios. |
...
## 16.105. Finalização local de Estruturas e Conversas — 2026-09-01

| Campo | Estado |
|---|---|
| Contrato | Objetivo: concluir a experiência local de Instituições, Unidades, Turmas, Atividades, Lançar/Fechar avaliações e Conversas. Incluído: `/dev`, buscas, paginação, arquivos visíveis, responsividade, mapas e correções descritas nos anexos. Fora deste gate Flutter: mutação remota, deploy e prova tenant A/B. Ordem: fixtures → UI → composição produtiva existente → regressão → documentação. Critério de parada: analyzer e testes focados verdes, sem sucesso falso ou fallback produtivo. Janela aceita: teto de 6 h, checkpoint em 3 h. |
| Fixtures `/dev` | 5 instituições, 1–4 unidades por instituição, até 28 turmas por unidade, 30 atividades e 12 modelos. Existem turmas com e sem atividades. Conversas usam repository local determinístico com busca, envio, leitura e estado limitado à sessão. Produção não reutiliza fixtures. |
| Buscas | Instituições, unidades, turmas, atividades, modelos, fechamento de avaliações e conversas filtram dados reais do repository local. Categorias mantém pesquisa interna. Cobertura nova executou unidades/turmas/atividades/chat 23/23; fechamento 1/1; busca e filtros de Instituições já permanecem cobertos pela suíte da tela. |
| UI concluída localmente | Cabeçalho compacto usa marca completa `Coelo` + chevron e drawer, sem hambúrguer; Bug permanece alcançável. Cards de Turmas usam grade 2×2 e `Professores` numérico. Instituições e Unidades exibem mapa no endereço. Arquivos ficam visíveis e honestos em todas as listagens do recorte. Modelos/Atividades usam tabs Todos/Ativos/Rascunho/Inativos; criar modelo abre página; duplicar preserva verbo, nome original e sufixo incremental. Fechamento e Conversas ganharam paginação inferior. Datas impedem ordem inválida e os dois campos deixaram de repetir `Publicar agora`. |
| Evidência | Analyzer focado de app: zero issues. Suítes: Atividades+Turmas 35/35; Unidades 39/39; avaliações+Chat 29/29; shell 64/64 após corrigir o flyout; `coelo_ui_admin` 110/110; formulário de Atividades 33/33; auth scope 5/5; rotas Chat 5/5. Testes focados de mapas, arquivos, buscas e matriz responsiva também verdes. |
| Progresso geral correto | O progresso Flutter geral continua sendo 102/207 `local-green` (49,28%), e não 0%. O valor 0/207 refere-se somente a `verified` ponta a ponta e não deve ser apresentado como progresso geral do Flutter. Este lote adiciona evidência e corrige regressões em ações já contabilizadas; não recalcula/promove a matriz sem auditoria por `action_id`. |
| Pendências reais | Duplicação de modelo para escopo de Unidade não pode ser concluída somente na UI: o domínio e o comando canônico aceitam apenas `platform`/`institution`. Produção de Unidades e Turmas permanece fail-closed no diretório porque faltam adapters/gateways produtivos completos. Inspeção humana final/goldens seletivos permanecem separados; nenhum golden foi atualizado automaticamente. |
| Conhecimento | Fonte canônica do router/spec e `docs/knowledge/team/superadmin-development-dataset.md` sincronizados com a escala atual e com a regra de busca funcional sem fallback produtivo. |

## 17. Histórico

| Data | Mudança |
|---|---|
| 2026-09-01 | Estruturas/Conversas finalizadas no gate Flutter local: fixtures 5/1–4/1–28/30/12, buscas funcionais, arquivos visíveis, mapas, shell compacto, cards de Turmas, modelos/atividades, avaliações e paginações; analyzer sem issues e regressões focadas verdes. Progresso Flutter geral preservado em 102/207 `local-green` (49,28%); adapters produtivos de Unidades/Turmas e escopo unitário de modelos continuam explícitos. |
