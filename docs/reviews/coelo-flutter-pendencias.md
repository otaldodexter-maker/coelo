...
|---:|---|---|---|
| 1 | `auth` — Login, recuperar, redefinir, MFA | `auth.login` `local-green`; `auth.recover` `local-green`; `auth.reset` `local-green`; `auth.logout` `local-green`; `auth.mfa` `audited` | Baseline Flutter Auth 126/126, delta final sessão/composição/Auth 56/56, oito goldens Recovery/Reset, build web release e `coelo_auth` 21/21; login/restauração exigem contexto do mesmo `session_id`. Ainda faltam produção, URL/redirect, SMTP/E2E e MFA; backend produtivo 1–2 d + E2E 0,5–1 d após URL. |
