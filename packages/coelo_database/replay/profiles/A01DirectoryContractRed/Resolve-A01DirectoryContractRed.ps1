...
function Assert-A01File([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
    throw "A01DirectoryContractRed input is missing: $Path"
  }
  $item = Get-Item -LiteralPath $Path -Force
  $cursor = $item
  while ($null -ne $cursor) {
    if (($cursor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
      throw "A01DirectoryContractRed input cannot contain a reparse point: $($cursor.FullName)"
    }
...
function Get-A01Hash([string]$Path) {
...
$descriptorFile = Assert-A01File (Join-Path $PSScriptRoot 'profile.json')
if ((Get-A01Hash $descriptorFile.FullName) -cne '7e417ac72bde47b1c84a6e2898076eada8763f77fff536328a2a7b842a521d28') {
  throw 'A01DirectoryContractRed descriptor hash mismatch'
}
$descriptor = [IO.File]::ReadAllText($descriptorFile.FullName) | ConvertFrom-Json
if ($TargetVersion -cne $descriptor.target_version) {
  throw "A01DirectoryContractRed requires target 20260901200206; received $TargetVersion"
}
$manifestFile = Assert-A01File (Join-Path $packageRoot 'replay\foundation-migrations.sha256')
if ((Get-A01Hash $manifestFile.FullName) -cne $descriptor.base.manifest_sha256_crlf_utf8) {
  throw 'A01DirectoryContractRed base manifest hash mismatch'
}
$baseEntries = @(Get-Content -LiteralPath $manifestFile.FullName | Where-Object {
  ...
  if ($_ -notmatch '^((\d{14})_[a-z0-9_]+\.sql)\|([0-9a-f]{64})$') {
    throw 'A01DirectoryContractRed base manifest entry is invalid'
  }
...
if ($baseEntries.Count -ne $descriptor.base.selected_canonical_count -or
  ...
  throw 'A01DirectoryContractRed requires Auth45 and zero extra bridges'
}
...
$versions = @($allEntries | ForEach-Object {
  if ($_.file -notmatch '^(\d{14})_[a-z0-9_]+\.sql$' -or
    ...
    throw 'A01DirectoryContractRed input name or hash is invalid'
  }
...
if ($canonicalEntries.Count -ne $descriptor.planned_counts.canonical -or
  ...
  throw 'A01DirectoryContractRed requires 52 unique canonical migrations and exactly two inherited preflights'
}
$canonical = @($canonicalEntries | ForEach-Object {
  $file = Assert-A01File (Join-Path (Join-Path $packageRoot 'migrations') $_.file)
  if ((Get-A01Hash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryContractRed input hash mismatch: $($_.file)"
  }
  $file
} | Sort-Object Name)
$preflight = @($preflightEntries | ForEach-Object {
  $file = Assert-A01File (Join-Path (Join-Path $packageRoot 'replay') $_.file)
  if ((Get-A01Hash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryContractRed input hash mismatch: $($_.file)"
  }
  $file
