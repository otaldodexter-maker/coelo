...
function Assert-A01AuditGreenFile([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
    throw "A01DirectoryAuditGreen input is missing: $Path"
  }
  $item = Get-Item -LiteralPath $Path -Force
  $cursor = $item
  while ($null -ne $cursor) {
    if (($cursor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
      throw "A01DirectoryAuditGreen input cannot contain a reparse point: $($cursor.FullName)"
    }
...
function Get-A01AuditGreenHash([string]$Path) {
...
# The reviewed A01 base54 is retained by exact names/hashes, with only the approved v2 added.
...
if ($TargetVersion -cne $descriptor.target_version) {
  throw "A01DirectoryAuditGreen requires target 20260907222911; received $TargetVersion"
}
$manifestFile = Assert-A01AuditGreenFile (Join-Path $packageRoot 'replay\foundation-migrations.sha256')
if ((Get-A01AuditGreenHash $manifestFile.FullName) -cne $descriptor.base.manifest_sha256_crlf_utf8) {
  throw 'A01DirectoryAuditGreen base manifest hash mismatch'
}
$baseEntries = @(Get-Content -LiteralPath $manifestFile.FullName | Where-Object {
  ...
  if ($_ -notmatch '^((\d{14})_[a-z0-9_]+\.sql)\|([0-9a-f]{64})$') {
    throw 'A01DirectoryAuditGreen base manifest entry is invalid'
  }
...
$versions = @($allEntries | ForEach-Object {
  if ($_.file -notmatch '^(\d{14})_[a-z0-9_]+\.sql$' -or
    ...
    throw 'A01DirectoryAuditGreen input name or hash is invalid'
  }
...
if ($canonicalEntries.Count -ne $descriptor.planned_counts.canonical -or
  ...
  throw 'A01DirectoryAuditGreen requires 53 unique canonical migrations and exactly two inherited preflights'
}
$canonical = @($canonicalEntries | ForEach-Object {
  $file = Assert-A01AuditGreenFile (Join-Path (Join-Path $packageRoot 'migrations') $_.file)
  if ((Get-A01AuditGreenHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryAuditGreen input hash mismatch: $($_.file)"
  }
  $file
} | Sort-Object Name)
$preflight = @($preflightEntries | ForEach-Object {
  $file = Assert-A01AuditGreenFile (Join-Path (Join-Path $packageRoot 'replay') $_.file)
  if ((Get-A01AuditGreenHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryAuditGreen input hash mismatch: $($_.file)"
  }
  $file
