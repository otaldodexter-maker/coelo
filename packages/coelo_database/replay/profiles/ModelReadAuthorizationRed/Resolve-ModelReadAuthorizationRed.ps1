...
function Assert-ModelReadFile([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
    throw "ModelReadAuthorizationRed input is missing: $Path"
  }
  $item = Get-Item -LiteralPath $Path -Force
  $cursor = $item
  while ($null -ne $cursor) {
    if (($cursor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
      throw "ModelReadAuthorizationRed input cannot contain a reparse point: $($cursor.FullName)"
    }
...
function Get-ModelReadHash([string]$Path) {
...
if ($TargetVersion -cne $descriptor.target_version) {
  throw "ModelReadAuthorizationRed requires target 20260901200206; received $TargetVersion"
}
$manifestFile = Assert-ModelReadFile (Join-Path $packageRoot 'replay\foundation-migrations.sha256')
if ((Get-ModelReadHash $manifestFile.FullName) -cne $descriptor.base.manifest_sha256_crlf_utf8) {
  throw 'ModelReadAuthorizationRed base manifest hash mismatch'
}
$baseEntries = @(Get-Content -LiteralPath $manifestFile.FullName | Where-Object {
  ...
  if ($_ -notmatch '^((\d{14})_[a-z0-9_]+\.sql)\|([0-9a-f]{64})$') {
    throw 'ModelReadAuthorizationRed base manifest entry is invalid'
  }
...
$versions = @($allEntries | ForEach-Object {
  if ($_.file -notmatch '^(\d{14})_[a-z0-9_]+\.sql$' -or
    ...
    throw 'ModelReadAuthorizationRed input name or hash is invalid'
  }
...
if ($canonicalEntries.Count -ne $descriptor.planned_counts.canonical -or
  ...
  throw 'ModelReadAuthorizationRed requires 47 unique canonical migrations and exactly two inherited preflights'
}
$canonical = @($canonicalEntries | ForEach-Object {
  $file = Assert-ModelReadFile (Join-Path (Join-Path $packageRoot 'migrations') $_.file)
  if ((Get-ModelReadHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "ModelReadAuthorizationRed input hash mismatch: $($_.file)"
  }
  $file
} | Sort-Object Name)
$preflight = @($preflightEntries | ForEach-Object {
  $file = Assert-ModelReadFile (Join-Path (Join-Path $packageRoot 'replay') $_.file)
  if ((Get-ModelReadHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "ModelReadAuthorizationRed input hash mismatch: $($_.file)"
  }
  $file
