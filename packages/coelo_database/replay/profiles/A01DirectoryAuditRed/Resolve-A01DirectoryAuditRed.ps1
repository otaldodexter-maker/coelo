[CmdletBinding()]
param([string]$TargetVersion = '20260907222911')

$ErrorActionPreference = 'Stop'
$packageRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))

function Assert-A01AuditFile([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
    throw "A01DirectoryAuditRed input is missing: $Path"
  }
  $item = Get-Item -LiteralPath $Path -Force
  $cursor = $item
  while ($null -ne $cursor) {
    if (($cursor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
      throw "A01DirectoryAuditRed input cannot contain a reparse point: $($cursor.FullName)"
    }
...
function Get-A01AuditHash([string]$Path) {
...
# The reviewed A01 base54 is retained by exact names/hashes, with only v1 added.
...
$descriptorFile = Assert-A01AuditFile (Join-Path $PSScriptRoot 'profile.json')
if ((Get-A01AuditHash $descriptorFile.FullName) -cne '1e3f5daa994a9a75174a4f78e330465cda24e6fa684f9bd799c4ab59399c5c07') {
  throw 'A01DirectoryAuditRed descriptor hash mismatch'
}
$descriptor = [IO.File]::ReadAllText($descriptorFile.FullName) | ConvertFrom-Json
if ($TargetVersion -cne $descriptor.target_version) {
  throw "A01DirectoryAuditRed requires target 20260907222911; received $TargetVersion"
}
$manifestFile = Assert-A01AuditFile (Join-Path $packageRoot 'replay\foundation-migrations.sha256')
if ((Get-A01AuditHash $manifestFile.FullName) -cne $descriptor.base.manifest_sha256_crlf_utf8) {
  throw 'A01DirectoryAuditRed base manifest hash mismatch'
}
$baseEntries = @(Get-Content -LiteralPath $manifestFile.FullName | Where-Object {
  ...
  if ($_ -notmatch '^((\d{14})_[a-z0-9_]+\.sql)\|([0-9a-f]{64})$') {
    throw 'A01DirectoryAuditRed base manifest entry is invalid'
  }
...
if ($baseEntries.Count -ne $descriptor.base.selected_canonical_count -or
  ...
  throw 'A01DirectoryAuditRed requires Auth46 and zero extra bridges'
}
...
$versions = @($allEntries | ForEach-Object {
  if ($_.file -notmatch '^(\d{14})_[a-z0-9_]+\.sql$' -or
    ...
    throw 'A01DirectoryAuditRed input name or hash is invalid'
  }
...
if ($canonicalEntries.Count -ne $descriptor.planned_counts.canonical -or
  ...
  throw 'A01DirectoryAuditRed requires 53 unique canonical migrations and exactly two inherited preflights'
}
$canonical = @($canonicalEntries | ForEach-Object {
  $file = Assert-A01AuditFile (Join-Path (Join-Path $packageRoot 'migrations') $_.file)
  if ((Get-A01AuditHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryAuditRed input hash mismatch: $($_.file)"
  }
  $file
} | Sort-Object Name)
$preflight = @($preflightEntries | ForEach-Object {
  $file = Assert-A01AuditFile (Join-Path (Join-Path $packageRoot 'replay') $_.file)
  if ((Get-A01AuditHash $file.FullName) -cne $_.sha256_crlf_utf8) {
    throw "A01DirectoryAuditRed input hash mismatch: $($_.file)"
  }
  $file
