...
begin
  ...
  if correlation is null then raise exception using message='activity audit correlation missing'; end if;
  select app_private.audit_append_superadmin_internal(
   ...
   p_permission_code,p_ctx.aal,p_action_code,'success',null,correlation,p_institution_id,
...
begin
  ...
  if permission_code is null then return app_private.activity_v2_error_envelope('ACTIVITY_INVALID_INPUT',p_correlation_id); end if;
  ...
  if fresh.internal_identity_id<>p_ctx.internal_identity_id or fresh.internal_auth_link_id<>p_ctx.internal_auth_link_id
    ...
    return app_private.activity_v2_error_envelope('SAI_PERMISSION_DENIED',p_correlation_id);
  end if;
  ...
  if r.institution_id is distinct from p_institution_id
    ...
    return app_private.activity_v2_error_envelope('SAI_CONCURRENT_CHANGE',p_correlation_id);
  end if;
  return app_private.activity_v2_success_envelope(pg_catalog.jsonb_build_object(
...
returns boolean language sql stable security definer set search_path='' as $$
 select jsonb_typeof(p_participants)='array' and jsonb_array_length(p_participants)<=1000
 and not exists(select 1 from jsonb_array_elements(p_participants) item
  where jsonb_typeof(item)<>'object' or item-array['group_id','child_group_link_id','belongs']<>'{}'::jsonb
   or jsonb_typeof(item->'belongs')<>'boolean'
   or not exists(select 1 from public.child_group_links cgl
 ...
 and (select count(*)=count(distinct (item->>'group_id',item->>'child_group_link_id')) from jsonb_array_elements(p_participants) item)
$$;

create function app_private.activity_v2_validate_professionals(p_institution_id uuid,p_professionals jsonb)
returns boolean language sql stable security definer set search_path='' as $$
 select jsonb_typeof(p_professionals)='array' and jsonb_array_length(p_professionals)<=500
 and not exists(select 1 from jsonb_array_elements(p_professionals) item
  where jsonb_typeof(item)<>'object' or item-array['membership_id','role','group_id']<>'{}'::jsonb
   ...
   or not exists(select 1 from public.institution_memberships m join public.people p on p.id=m.person_id
    where m.id=(item->>'membership_id')::uuid and m.institution_id=p_institution_id
      and m.status='active' and m.revoked_at is null and p.person_type<>'child' and p.status='active'))
 and (select count(*)=count(distinct (item->>'membership_id',item->>'role',coalesce(item->>'group_id',''))) from jsonb_array_elements(p_professionals) item)
$$;
...
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid(); result jsonb;
 search_text text; institution_filter uuid; status_filter text; unit_filter uuid; group_filter uuid;
begin
 begin
  select * into strict ctx from app_private.activity_v2_require_context('activities.read',null);
  if ctx.aal<>'aal2' then raise insufficient_privilege using detail='SAI_MFA_REQUIRED'; end if;
  ...
  select pg_catalog.jsonb_build_object('items',coalesce(jsonb_agg(pg_catalog.jsonb_build_object(
   ...
   'created_at',p.created_at,'updated_at',p.updated_at) order by p.name),'[]'::jsonb),
 ...
 exception when others then get stacked diagnostics status_filter=pg_exception_detail;
  return app_private.activity_v2_error_envelope(coalesce(nullif(status_filter,''),'SAI_INTERNAL_ERROR'),correlation); end;
end $$;
...
begin begin
 ...
 select * into strict ctx from app_private.activity_v2_require_context('activities.read',null);
 if ctx.aal<>'aal2' then raise insufficient_privilege using detail='SAI_MFA_REQUIRED'; end if;
 if ctx.scope_kind='institution' and ctx.scope_institution_id is distinct from a.institution_id then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
 ...
 if 'participants'=any(p_sections) then result:=result||pg_catalog.jsonb_build_object('participants',coalesce((select jsonb_agg(pg_catalog.jsonb_build_object('child_group_link_id',p.child_group_link_id,'group_id',gl.group_id,'display_name',person.display_name,'status',p.status)) from public.activity_group_participants p join public.activity_group_links gl on gl.id=p.activity_group_link_id join public.child_group_links cgl on cgl.id=p.child_group_link_id join public.child_unit_links cul on cul.id=cgl.child_unit_link_id join public.child_contexts cc on cc.id=cul.child_context_id join public.people person on person.id=cc.child_person_id where gl.activity_id=a.id and p.status='active'),'[]'::jsonb)); end if;
 if 'professionals'=any(p_sections) then result:=result||pg_catalog.jsonb_build_object('professionals',coalesce((select jsonb_agg(item) from(
  select pg_catalog.jsonb_build_object('membership_id',x.membership_id,'role','instructor','group_id',gl.group_id,'display_name',p.display_name,'status',x.status) item from public.activity_group_assignments x join public.activity_group_links gl on gl.id=x.activity_group_link_id join public.people p on p.id=x.person_id where gl.activity_id=a.id and x.status='active' and x.assignment_role='instructor'
  union all select pg_catalog.jsonb_build_object('membership_id',x.membership_id,'role','activity_admin','group_id',null,'display_name',p.display_name,'status',x.status) from public.activity_admin_assignments x join public.people p on p.id=x.person_id where x.activity_id=a.id and x.status='active') q),'[]'::jsonb)); end if;
 if 'permissions'=any(p_sections) then result:=result||pg_catalog.jsonb_build_object('permissions',pg_catalog.jsonb_build_object(
  ...
  'professional_actions','[]'::jsonb)); end if;
 return app_private.activity_v2_success_envelope(result);
 exception when others then get stacked diagnostics code=pg_exception_detail; return app_private.activity_v2_error_envelope(coalesce(nullif(code,''),'SAI_INTERNAL_ERROR'),correlation); end; end $$;
...
begin begin
 ...
 if ctx.aal<>'aal2' then raise insufficient_privilege using detail='SAI_MFA_REQUIRED'; end if;
 if p_limit not between 1 and 100 or length(coalesce(p_search,''))>120 or coalesce(array_length(p_sections,1),0)<>(select count(distinct x) from unnest(p_sections)x)
 ...
 if 'participants'=any(p_sections) then result:=result||pg_catalog.jsonb_build_object('participants',coalesce((select jsonb_agg(pg_catalog.jsonb_build_object('child_group_link_id',cgl.id,'group_id',cgl.group_id,'display_name',p.display_name)) from public.child_group_links cgl join public.child_unit_links cul on cul.id=cgl.child_unit_link_id and cul.status='active' join public.child_contexts cc on cc.id=cul.child_context_id and cc.status='active' join public.people p on p.id=cc.child_person_id and p.status='active' join public.groups g on g.id=cgl.group_id and g.institution_id=p_institution_id where cgl.status='active' and (p_search is null or p.display_name ilike '%'||p_search||'%') limit p_limit),'[]'::jsonb)); end if;
 if 'professionals'=any(p_sections) then result:=result||pg_catalog.jsonb_build_object('professionals',coalesce((select jsonb_agg(pg_catalog.jsonb_build_object('membership_id',m.id,'display_name',p.display_name,'role_code',m.role_code)) from public.institution_memberships m join public.people p on p.id=m.person_id where m.institution_id=p_institution_id and m.status='active' and m.revoked_at is null and p.person_type<>'child' and p.status='active' and (p_search is null or p.display_name ilike '%'||p_search||'%') limit p_limit),'[]'::jsonb)); end if;
...
begin begin
 ...
 or jsonb_typeof(p_payload->'unit_ids')<>'array' or length(btrim(coalesce(p_payload->>'name',''))) not between 1 and 120
 or length(coalesce(p_payload->>'description',''))>500 or length(btrim(coalesce(p_payload->>'initials',''))) not between 1 and 2
 ...
 if p_request_id is null or nullif(btrim(p_payload->>'name'),'') is null or coalesce(array_length(units,1),0) not between 1 and 100 then raise invalid_parameter_value using detail='ACTIVITY_INVALID_INPUT'; end if;
...
begin begin
 if p_payload is null or jsonb_typeof(p_payload)<>'object' or p_payload-array['name','description','taxonomy_id','icon_key','initials']<>'{}'::jsonb then raise invalid_parameter_value using detail='ACTIVITY_INVALID_INPUT'; end if;
 select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
...
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid(); a public.activity_definitions%rowtype; h bytea; replay jsonb; code text;
begin begin select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
...
begin begin select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
 ...
 if not app_private.activity_v2_validate_participants(a.institution_id,p_participants) then raise invalid_parameter_value using message='duplicate participant or invalid chain (max 1000)',detail='ACTIVITY_INVALID_INPUT'; end if;
...
begin begin select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
 select * into strict ctx from app_private.activity_v2_require_context('activities.assign_people',a.institution_id);
 if not app_private.activity_v2_validate_professionals(a.institution_id,p_professional_assignments) then raise invalid_parameter_value using message='duplicate professional or invalid active membership (max 500)',detail='ACTIVITY_INVALID_INPUT'; end if;
...
begin begin select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if;
 ...
 if jsonb_typeof(p_capability_policies)<>'object' or (select array_agg(key order by key) from jsonb_object_keys(p_capability_policies)key)<>approved
 ...
 or jsonb_typeof(p_group_capability_settings)<>'array' or jsonb_typeof(p_professional_capability_actions)<>'array' or jsonb_array_length(p_professional_capability_actions)>500
 or exists(select 1 from jsonb_array_elements(p_group_capability_settings)x where x-array['group_id','capabilities']<>'{}'::jsonb or jsonb_typeof(x->'capabilities')<>'object' or (select array_agg(key order by key) from jsonb_object_keys(x->'capabilities')key)<>approved or exists(select 1 from jsonb_each(x->'capabilities')v where v.value<>'null'::jsonb and jsonb_typeof(v.value)<>'boolean'))
 or exists(select 1 from jsonb_array_elements(p_professional_capability_actions)x where x-array['membership_id','role','group_id','actions']<>'{}'::jsonb or x->>'role' not in('instructor','activity_admin') or (x->>'role'='instructor' and nullif(x->>'group_id','') is null) or (x->>'role'='activity_admin' and x->'group_id'<>'null'::jsonb) or jsonb_typeof(x->'actions')<>'object' or (select array_agg(key order by key) from jsonb_object_keys(x->'actions')key)<>approved or exists(select 1 from jsonb_each_text(x->'actions')v where v.value not in('none','view','edit','both')))
 ...
 for item in select element.value from jsonb_array_elements(p_professional_capability_actions) element loop if item->>'role'='activity_admin' then select x.id into ass from public.activity_admin_assignments x where x.activity_id=a.id and x.membership_id=(item->>'membership_id')::uuid and x.status='active'; else select x.id into ass from public.activity_group_assignments x join public.activity_group_links y on y.id=x.activity_group_link_id where y.activity_id=a.id and y.group_id=(item->>'group_id')::uuid and x.membership_id=(item->>'membership_id')::uuid and x.status='active'; end if; if ass is null then raise foreign_key_violation using detail='ACTIVITY_INVALID_REFERENCE'; end if; for entry in select * from jsonb_each_text(item->'actions') loop select c.id into cap from public.activity_capabilities c where c.code=entry.key; if item->>'role'='activity_admin' then insert into public.activity_admin_capability_actions(activity_admin_assignment_id,capability_id,can_view,can_edit,changed_by_person_id) values(ass,cap,entry.value in('view','both'),entry.value in('edit','both'),null); else insert into public.activity_assignment_capability_actions(assignment_id,capability_id,can_view,can_edit,changed_by_person_id) values(ass,cap,entry.value in('view','both'),entry.value in('edit','both'),null); end if; end loop; end loop;
...
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid(); a public.activity_definitions%rowtype; h bytea; replay jsonb; code text;
begin begin select * into a from public.activity_definitions x where x.id=p_activity_id; if a.id is null then raise no_data_found using detail='ACTIVITY_NOT_FOUND'; end if; select * into strict ctx from app_private.activity_v2_require_context('activities.manage',a.institution_id); h:=app_private.activity_v2_command_request_hash('activity.publish',a.institution_id,a.id,p_expected_version,'{}'); replay:=app_private.activity_v2_replay_or_error(ctx,p_request_id,a.institution_id,a.id,'activity.publish',h,correlation); if replay is not null then return replay; end if;
 select * into a from public.activity_definitions x where x.id=p_activity_id for update; if a.management_version<>p_expected_version then raise serialization_failure using detail='SAI_CONCURRENT_CHANGE'; end if; if a.status<>'draft' or a.taxonomy_id is null then raise invalid_parameter_value using detail='ACTIVITY_INVALID_STATE'; end if;
 if not exists(select 1 from public.activity_unit_links x where x.activity_id=a.id and x.status='active')
 or not exists(select 1 from public.activity_group_links x where x.activity_id=a.id and x.status='active')
 or exists(select 1 from public.activity_group_assignments ga join public.activity_group_links gl on gl.id=ga.activity_group_link_id where gl.activity_id=a.id and ga.status='active' and ga.revoked_at is null and ga.assignment_role='instructor' and (select count(*) from public.activity_assignment_capability_actions x where x.assignment_id=ga.id)<>5)
 or exists(select 1 from public.activity_admin_assignments aa where aa.activity_id=a.id and aa.status='active' and aa.revoked_at is null and (select count(*) from public.activity_admin_capability_actions x where x.activity_admin_assignment_id=aa.id)<>5)
