-- 20260911190200_circulars_media_private_r2_v1_reapply
--
-- Reaplicacao, na faixa de publicacoes-agenda (R05, 11/09/2026), da migration
-- historica 20260909212000_circulars_media_private_r2_v1 (E2 R02/L01), que
-- NAO esta em producao: a prova de circulars.attach com a sessao qa-r03 em
-- 11/09 14:18 devolveu bucket_id 'coelo-circulars-private' e o PUT foi para o
-- Storage do Supabase (prepare_circular_media_upload de producao nao conhece
-- storage_provider; default da coluna = 'supabase'). A pendencia do rastreador
-- ("presente e fora da fila noturna, 6/6") media presenca de funcoes, nao a
-- forma R2. Sem este pacote, circular-media v5 usa o ramo legado e a ADR 0032
-- (toda midia nova do MVP em R2) nao vale para Circulares.
--
-- Diferenca em relacao ao texto historico: os quatro CHECKs novos sao
-- descartados antes de recriados (idempotencia), o resto e identico.
-- pgTAP: circulars_media_private_r2_v1_test 45/46 no descartavel (o 46o exige a
-- linha do bucket legado em storage.buckets, que o dump nao carrega; em
-- producao o bucket existe) + circular_actor_internal_bridge_v1_test 14/14 apos.
--
...
-- Idempotencia: os CHECKs novos sao recriados a partir do zero.
alter table public.circular_media_assets
  drop constraint if exists circular_media_assets_storage_provider_ck,
  drop constraint if exists circular_media_assets_bucket_ck,
  drop constraint if exists circular_media_assets_r2_key_shape_ck,
  drop constraint if exists circular_media_assets_checksum_format_ck,
  drop constraint if exists circular_media_assets_r2_ready_integrity_ck;
