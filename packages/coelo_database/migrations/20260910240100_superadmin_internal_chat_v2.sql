-- Chat interno do Superadmin (realm interno v2), sobre a baseline de producao.
--
-- 20260901101500_superadmin_internal_chat_v2 nunca chegou a producao: inseria
-- em platform_permissions sem os tres labels NOT NULL e chamava
-- audit_append_superadmin_internal com um 14o argumento jsonb que a funcao de
-- producao (13 argumentos) nao tem. Este pacote reescreve aquele arquivo para a
-- forma canonica de producao:
--   - labels fornecidos; requires_mfa=false (ADR 0034, Decisao 12: nenhuma
--     capacidade exige segundo fator no MVP);
--   - auditoria com a assinatura de 13 argumentos;
--   - revoke explicito de PUBLIC/anon/authenticated/service_role em toda
--     funcao nova antes do grant minimo a authenticated.
-- Exige 20260910240000 (chat_attachment_metadata e indice de cursor).
begin;
...
begin
  ...
  if to_regprocedure('app_private.require_superadmin_internal_context(text)') is null
     ...
     or to_regclass('app_private.superadmin_internal_memberships') is null
...
-- 2. Mensagem com autor do realm interno (sem pessoa).
alter table public.messages alter column author_person_id drop not null;
alter table public.messages
  add column if not exists author_kind text not null default 'person',
...
do $$
begin
  if not exists(select 1 from pg_constraint where conname='messages_author_realm_check'
      and conrelid='public.messages'::regclass) then
    alter table public.messages add constraint messages_author_realm_check check(
...
create or replace function app_private.guard_chat_message_internal_author()
...
drop trigger if exists messages_internal_author_guard on public.messages;
...
-- 3. Recibos e idempotencia do realm interno, sem caminho direto de cliente.
create table if not exists app_private.superadmin_internal_chat_receipts(
...
create index if not exists superadmin_internal_chat_receipts_unread_idx
  on app_private.superadmin_internal_chat_receipts(internal_identity_id,message_id)
  where read_at is null;

create table if not exists app_private.superadmin_internal_chat_command_receipts(
...
-- 4. Envelope spec-039 do chat interno.
create or replace function app_private.superadmin_chat_success(p_data jsonb)
...
-- 5. Gateways do realm interno (security definer; contexto, capacidade e
--    escopo revalidados a cada chamada; auditoria com 13 argumentos).
...
create or replace function public.superadmin_chat_unread_total_v2()
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid();
...
create or replace function public.superadmin_chat_send_message_v2(
...
begin begin
 ...
 if prior.request_id is not null then
  ...
  perform app_private.audit_append_superadmin_internal(ctx.internal_identity_id,
    ...
    institution,'message',created_message.id);
 end if;
...
create or replace function public.superadmin_chat_mark_read_v2(p_conversation_id uuid,p_through_message_id uuid)
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid();
...
create or replace function public.superadmin_chat_realtime_refresh_v2(p_conversation_id uuid)
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare ctx app_private.superadmin_internal_context; correlation uuid:=gen_random_uuid();
...
-- 6. O realm de pessoas tambem recebe e reconhece mensagens do realm interno.
--    `IS DISTINCT FROM` e intencional: mensagem interna nao tem autor pessoa.
...
begin
 ...
 filtered as(select * from candidate candidate_row where(not p_unread_only or candidate_row.unread_count>0)
  and(p_cursor_activity_at is null or(candidate_row.activity_at,candidate_row.id)<(p_cursor_activity_at,p_cursor_conversation_id))
  order by candidate_row.activity_at desc,candidate_row.id desc limit p_limit)
 ...
 from filtered filtered_row order by filtered_row.activity_at desc,filtered_row.id desc;
end $$;
...
-- 7. Privilegios: revoke explicito antes do grant minimo.
