-- R13 / ADR 0038 H21: 4.000 caracteres e o total da circular somando os blocos de
-- texto (antes 10.000). Mesmo corpo do save_draft_v2 do lote de midia (190300), so o
-- limite muda; a constraint da tabela acompanha (maior corpo em producao: 83).
alter table public.circular_revisions drop constraint if exists circular_revisions_body_length_check;
alter table public.circular_revisions add constraint circular_revisions_body_length_check check (char_length(body_text) <= 4000);
