-- Candidato principal-chat-sistema 20260910191000 (R04, retomada coelo-11).
-- Recarimbo da historica 20260901161700_principal_runtime_contexts.sql, que
-- nunca chegou a baseline: em producao (11/09/2026 03:40) PostgREST devolve
-- PGRST202 para public.list_my_principal_contexts e as cinco telas do
-- Principal (Acontece, Momentos, Agora, Para voce, Perfil) param em
-- "Nao conseguimos validar seu contexto agora". Conteudo identico a historica;
-- idempotente (create or replace + revoke/grant). Depende de
-- person_auth_links, people, institution_memberships, institutions, groups e
-- units, todas presentes em producao (conferido por pg_class em 03:50).
-- pgTAP: supabase/tests/principal_runtime_contexts_test.sql (13 casos, ja no
-- repositorio; nao rodado no descartavel nesta retomada por memoria).
-- Nao cria contexto para ninguem: o resolvedor so lista memberships
-- institucionais ativas do proprio usuario; qa-r03 continuara sem contexto
-- ("Nenhum contexto disponivel", honesto) ate existir membership institucional
-- para a pessoa de servico (P25).
