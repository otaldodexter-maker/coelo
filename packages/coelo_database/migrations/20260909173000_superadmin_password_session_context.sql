-- source: RED local-auth-recovery-boundary.log; approved password login contract;
--         20260901200206_defer_superadmin_internal_mfa_until_mvp_go_live.sql
-- status: proposed-local-only; not authorized for remote application
...
begin
  if current_user <> 'postgres' then
    raise exception using
      errcode = '42501',
      message = 'internal auth password-session policy migration must run as postgres';
  end if;
  ...
  if pg_catalog.to_regclass('auth.mfa_amr_claims') is null
     or not exists (select 1 from information_schema.columns
       where table_schema = 'auth' and table_name = 'mfa_amr_claims'
         and column_name = 'session_id' and udt_name = 'uuid')
     or not exists (select 1 from information_schema.columns
       where table_schema = 'auth' and table_name = 'mfa_amr_claims'
         and column_name = 'authentication_method' and udt_name in ('text', 'varchar')) then
    raise exception using errcode = 'P0001',
      message = 'provider-owned session AMR schema drift';
  end if;
  ...
  if pg_catalog.strpos(function_definition, 'SAI_MFA_REQUIRED') <> 0
     or pg_catalog.strpos(function_definition, 'auth.mfa_amr_claims') <> 0
...
begin
  ...
  -- The provider-owned AMR row belongs to the already validated session.
  -- OTP/recovery alone must not authorize the password-based Superadmin app.
  -- Auth recovery endpoints remain available; no MFA gate is introduced.
  if not exists (
    select 1 from auth.mfa_amr_claims amr_claim
    where amr_claim.session_id = current_session_id
      and amr_claim.authentication_method = 'password'
  ) then
    raise insufficient_privilege using message = 'internal authorization denied',
      detail = 'SAI_SESSION_INVALID';
  end if;
...
begin
  ...
  if pg_catalog.strpos(function_definition, 'SAI_MFA_REQUIRED') <> 0
     or pg_catalog.strpos(function_definition, 'auth.mfa_amr_claims') = 0
     or pg_catalog.strpos(function_definition, 'amr_claim.session_id = current_session_id') = 0
     or pg_catalog.strpos(function_definition, 'amr_claim.authentication_method = ''password''') = 0
     or pg_catalog.strpos(function_definition, 'auth.sessions') = 0
     or pg_catalog.strpos(function_definition, 'auth.users') = 0
     or pg_catalog.strpos(function_definition, 'platform_role_permissions') = 0
     or pg_catalog.strpos(function_definition, 'permission_record.requires_mfa') = 0 then
    raise exception using errcode = 'P0001',
      message = 'internal auth password-session policy postcondition failed';
  end if;
