...
import {
  type HappensMediaDependencies,
  handleHappensMediaRequest,
} from "./index.ts";
import { R2TransportError } from "../_shared/r2_s3.ts";

// Ramo R2 do gateway de midia do Acontece.
//
...
//
// RETIDO DE PROPOSITO: `prepare_happens_media_upload` ainda NAO devolve
...
const environment: Record<string, string> = {
  HAPPENS_MEDIA_ALLOWED_ORIGINS: "https://admin.coelo.test",
...
function dependenciesFor(
  ...
  return {
    ...
    createClient: ((_url: string, key: string) => {
      ...
      return {
        ...
        rpc: (name: string) => {
          if (name === "finalize_happens_media_upload") {
            return Promise.resolve({ data: { ok: true }, error: null });
          }
          return Promise.resolve({ data: descriptor, error: null });
        },
      };
    }) as unknown as HappensMediaDependencies["createClient"],
  ...
  } as unknown as HappensMediaDependencies;
}
...
function post(body: Record<string, unknown>) {
  return new Request("https://project.functions.supabase.co/happens-media", {
    method: "POST",
    headers: {
...
const r2Descriptor = {
  ...
  object_key: "happens/institution-1/post-1/asset-1.png",
  mime_type: "image/png",
};

const legacyDescriptor = {
  asset_id: "asset-1",
  bucket_id: "coelo-happens-mvp",
  object_key: "institution-1/post-1/asset-1.png",
  mime_type: "image/png",
};

Deno.test("prepares an R2 upload with a short lived signed PUT", async () => {
  const tracked = probe();
  const response = await handleHappensMediaRequest(
    post(uploadBody),
...
Deno.test("signs an R2 read and leaves the legacy read alone", async () => {
  const onR2 = probe();
  const r2Response = await handleHappensMediaRequest(
  ...
  const legacy = probe();
  const legacyResponse = await handleHappensMediaRequest(
    post({ action: "read", read_ticket: "ticket-1" }),
    dependenciesFor(legacyDescriptor, legacy),
  );
  assertEquals(legacyResponse.status, 200);
  assertEquals(legacy.storageCalls, ["read:coelo-happens-mvp"]);
...
Deno.test("discards an R2 object whose bytes fail the MIME signature", async () => {
  ...
  assertEquals(response.status, 422);
  assertEquals(body.error, "invalid_media_signature");
  assertEquals(tracked.r2Calls.includes(`delete:${r2Descriptor.object_key}`), true);
});

Deno.test("deletes through the provider the descriptor names", async () => {
  const onR2 = probe();
  await handleHappensMediaRequest(
    post({ action: "delete", asset_id: "asset-1" }),
    dependenciesFor(r2Descriptor, onR2),
  );
  assertEquals(onR2.r2Calls, [`delete:${r2Descriptor.object_key}`]);
  assertEquals(onR2.storageCalls, []);
  ...
  assertEquals(legacy.storageCalls, ["remove:coelo-happens-mvp"]);
  assertEquals(legacy.r2Calls, []);
});
...
Deno.test("still rejects an unauthenticated POST before touching anything", async () => {
  ...
  const request = new Request(
    "https://project.functions.supabase.co/happens-media",
    {
      method: "POST",
