...
export function authorizeUploadFailureJob(jobId: string, contract: RpcResult) {
  if (contract.error) throw new Error("upload_unauthorized");
  return { jobId, data: contract.data as Json };
}

export async function recordAuthorizedFailureBestEffort(
  jobId: string | null,
  ...
  try {
    ...
    // The authorized job remains auditable if failure recording is unavailable.
  }
}
...
function cors(origin: string | null): HeadersInit {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, apikey, content-type, x-client-info, x-coelo-import-action, x-coelo-import-job-id",
...
function column(value: unknown) {
  const normalized = String(value ?? "").normalize("NFD").replace(
    /[\u0300-\u036f]/g,
    "",
...
function rowsFromMatrix(matrix: unknown[][]): Json[] {
  ...
  if (
    headers.some((item) => !item || !allowed.has(item)) ||
    new Set(headers).size !== headers.length
  ) throw new Error("invalid_headers");
  return matrix.filter((row) => row.some((item) => String(item ?? "").trim()))
    .map((row) =>
      Object.fromEntries(
        headers.map((
          header,
...
function xlsx(bytes: Uint8Array) {
  ...
  return rowsFromMatrix(
    XLSX.utils.sheet_to_json<unknown[]>(book.Sheets[first], {
      header: 1,
      defval: "",
      raw: false,
      blankrows: false,
...
async function unitExport(url: string, authorization: string, body: Json) {
  ...
  if (!delegated.ok) {
    throw new Error(String(payload.error ?? "unit_export_failed"));
  }
  return payload;
}
...
export async function handler(request: Request) {
  ...
  try {
    ...
    if (action === "upload") {
      const uploadJobId = request.headers.get("x-coelo-import-job-id");
      if (!uploadJobId || !UUID.test(uploadJobId)) {
        return response(origin, 400, { error: "invalid_request" });
      }
      const contract = await user.rpc(
        "superadmin_import_export_upload_contract",
        { p_import_job_id: uploadJobId },
      );
      const authorized = authorizeUploadFailureJob(uploadJobId, contract);
      authorizedFailureJobId = authorized.jobId;
      const upload = authorized.data;
      const mime = String(upload.mime_type ?? "");
      if (request.headers.get("content-type")?.split(";", 1)[0] !== mime) {
      ...
      if (stored.error) {
        if (!stored.error.message.toLowerCase().includes("already exists")) {
    ...
    if (body.action === "confirm_import" || body.action === "retry_import") {
      ...
      const result = await user.rpc(
        ...
        { p_import_job_id: body.job_id, p_request_id: body.request_id },
      );
      if (result.error) throw new Error("job_transition_failed");
      return response(origin, 200, result.data as Json);
    }
    if (body.action === "request_export") {
      const created = await user.rpc("superadmin_request_import_export", {
        p_domain: body.domain,
      ...
      return response(
        origin,
        200,
        await unitExport(url, authorization, {
          action: "generate",
          format: body.format,
    ...
    if (body.action === "status" || body.action === "download") {
      ...
      const result = await user.rpc("superadmin_get_import_export_job", {
        p_import_job_id: body.job_id,
      });
