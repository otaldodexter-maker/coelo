...
export async function recordCreatedFailureBestEffort(
  ...
  try {
    ...
    // Best effort state transition; the authorized job remains auditable.
  }
}
...
function matrixRows(matrix: unknown[][]): Json[] {
  ...
  if (
    columns.some((value) => !value || !allowed.has(value)) ||
...
export async function handler(request: Request) {
  ...
  try {
    ...
    if (action === "status") {
      ...
      const result = await user.rpc("superadmin_get_unit_file_job", {
    ...
    if (action === "confirm" || action === "retry") {
      ...
      const result = await user.rpc(
        ...
        { p_request_id: body.request_id, p_import_job_id: requestedJobId },
      );
    ...
    const fileName = typeof body.file_name === "string"
      ? body.file_name.trim()
      : "";
    const format = fileName.toLowerCase().endsWith(".csv")
      ? "csv"
    ...
    if (!rows.length || rows.length > MAX_ROWS) {
      return reply(origin, 400, { error: "invalid_row_count" });
    }
    ...
    createdJobId = captureCreatedJobId(created, "job_create_failed");
    ...
    if (upload.error) {
      if (!upload.error.message.toLowerCase().includes("already exists")) {
        throw new Error("storage_upload_failed");
      }
      const existing = await admin.storage.from(BUCKET).download(path);
      if (
        existing.error ||
        await checksum(new Uint8Array(await existing.data.arrayBuffer())) !==
          digest
      ) {
