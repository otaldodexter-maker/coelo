...
select ok((select relrowsecurity and relforcerowsecurity from pg_class where oid='public.notice_events'::regclass),'notice events force RLS');
...
select ok(not has_table_privilege('authenticated','public.notice_events','select,insert,update,delete'),'frontend cannot access notice events directly');
...
select ok(
  position('p_limit+1' in pg_get_functiondef('public.list_notices_for_superadmin(text,text[],text[],timestamptz,uuid,int)'::regprocedure))>0,
...
select ok(
  position("status::text in ('active','trial')" in pg_get_functiondef('app_private.materialize_notice_publication_job(uuid,int)'::regprocedure))>0,
...
select '82100000-0000-4000-8000-000000000001',id,'active','platform',true
from public.platform_roles where code='owner';

create temporary table notice_test_inputs(payload jsonb not null,notice_id uuid);
grant select,update on notice_test_inputs to authenticated;
