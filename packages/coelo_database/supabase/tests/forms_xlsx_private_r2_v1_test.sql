...
select throws_ok($$select pg_temp.xlsx_begin('other-worker')$$,'PT409','forms_xlsx_lease_unavailable','wrong worker cannot begin an existing lease');
select throws_ok($$select pg_temp.xlsx_page('c02-xlsx-1','8c021000-0000-4000-8000-000000000800',0)$$,'PT409','forms_xlsx_attempt_unavailable','asset ID from another job cannot page this snapshot');
...
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot','{}','{"attempt":2}')$$,'PT409','forms_xlsx_multipart_scope_mismatch','attempt number cannot be forged');
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot','{}','{"bucket":"coelo-media-prod"}')$$,'PT409','forms_xlsx_multipart_scope_mismatch','bucket cannot be substituted');
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot','{}','{"object_key":"forged/path"}')$$,'PT409','forms_xlsx_multipart_scope_mismatch','opaque catalog key cannot be substituted');
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot','{}','{"snapshot_row_count":99}')$$,'PT409','forms_xlsx_multipart_scope_mismatch','sealed row count cannot be substituted');
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot','{}','{"extra":true}')$$,'PT409','forms_xlsx_multipart_scope_mismatch','unknown scope fields rejected');
...
select throws_ok($$select pg_temp.xlsx_mp('begin1','begin','{"upload_id":"other-upload"}')$$,'PT409','forms_xlsx_multipart_begin_mismatch','lost begin cannot silently replace upload ID');
select throws_ok($$select pg_temp.xlsx_mp('begin1','record_part',pg_temp.xlsx_mp_part('upload-1',2,5242880))$$,'PT409','forms_xlsx_multipart_part_out_of_sequence','parts must arrive in order');
...
select throws_ok($$select pg_temp.xlsx_mp('begin1','record_part',pg_temp.xlsx_mp_part('upload-1',1,5242881))$$,'PT409','forms_xlsx_multipart_part_replay_mismatch','part retry with other bytes rejected');
...
set local role service_role;
select throws_ok($$select pg_temp.xlsx_begin('c02-xlsx-1')$$,'PT409','forms_xlsx_lease_unavailable','expired lease cannot reserve another asset');
reset role;
...
select throws_ok($$select pg_temp.xlsx_mp('begin1','snapshot')$$,'PT409','forms_xlsx_lease_unavailable','prior worker cannot resume after takeover');
...
select throws_ok($$select pg_temp.xlsx_mp('begin2','complete',pg_temp.xlsx_mp_digest('upload-2',100,repeat('d',64)))$$,'PT409','forms_xlsx_multipart_complete_mismatch','another integral checksum cannot replace sealed receipt');
...
select throws_ok($$select pg_temp.xlsx_page('c02-xlsx-1',(body->>'asset_id')::uuid,0) from xlsx_worker_results where label='begin1'$$,'PT409','forms_xlsx_lease_unavailable','late old worker cannot read after lease takeover');
select throws_ok($$select pg_temp.xlsx_page('c02-xlsx-2',(body->>'asset_id')::uuid,0) from xlsx_worker_results where label='begin1'$$,'PT409','forms_xlsx_attempt_unavailable','new worker cannot revive an abandoned attempt');
reset role;
...
select throws_ok($$select pg_temp.xlsx_finish('c02-xlsx-1',(body->>'asset_id')::uuid,100,repeat('c',64)) from xlsx_worker_results where label='begin1'$$,'PT409','forms_xlsx_lease_unavailable','stale worker cannot publish its abandoned upload');
reset role;
...
select throws_ok($$select pg_temp.xlsx_finish('c02-xlsx-2',(body->>'asset_id')::uuid,101,repeat('d',64)) from xlsx_worker_results where label='begin2'$$,'PT409','forms_xlsx_lease_unavailable','completed lease cannot rewrite winner with another body');
reset role;
...
select throws_ok($$select public.form_worker_complete_cleanup('8c021000-0000-4000-8000-000000014000','c02-legacy-cleanup',array['8c021000-0000-4000-8000-000000000712'::uuid])$$,'PT409','cleanup items unavailable','Storage cleanup cannot claim a forged R2 job ID');
reset role;
