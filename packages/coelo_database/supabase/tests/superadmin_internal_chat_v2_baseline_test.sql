-- Prova do pacote 20260910240100 (chat interno v2 sobre a baseline).
-- Sessoes AAL1: no MVP nenhuma capacidade exige segundo fator (ADR 0034, D12).
begin;
...
select ok((select bool_and(not requires_mfa and module_label<>'' and screen_label<>'' and action_label<>'')
  from public.platform_permissions where code in('chat.internal.read','chat.internal.send')),
 'internal chat capabilities carry labels and require no second factor in the MVP');
...
insert into auth.sessions(id,user_id,created_at,updated_at,aal,not_after) values
 ('9c100000-0000-4000-8000-000000000201','9c100000-0000-4000-8000-000000000101',now(),now(),'aal1',now()+interval '1 hour'),
 ('9c100000-0000-4000-8000-000000000202','9c100000-0000-4000-8000-000000000102',now(),now(),'aal1',now()+interval '1 hour'),
 ('9c100000-0000-4000-8000-000000000204','9c100000-0000-4000-8000-000000000104',now(),now(),'aal1',now()+interval '1 hour');
insert into app_private.superadmin_internal_identities(id) values
 ('9c100000-0000-4000-8000-000000000301'),('9c100000-0000-4000-8000-000000000302'),
 ('9c100000-0000-4000-8000-000000000304');
...
select set_config('request.jwt.claims',jsonb_build_object('sub','9c100000-0000-4000-8000-000000000101','session_id','9c100000-0000-4000-8000-000000000201','aal','aal1','role','authenticated')::text,true);
insert into chat_results values
 ...
 ('owner_refresh',public.superadmin_chat_realtime_refresh_v2('9c100000-0000-4000-8000-000000000701'));
...
select set_config('request.jwt.claims',jsonb_build_object('sub','9c100000-0000-4000-8000-000000000104','session_id','9c100000-0000-4000-8000-000000000204','aal','aal1','role','authenticated')::text,true);
...
select ok((select body#>>'{ok}'='true' and body#>>'{data,author_name}'='Equipe Coelo'
 from chat_results where label='owner_send'),'aal1 owner sends as the internal realm (MFA deferred)');
...
select ok((select body#>>'{data,unread_count}'='0' and body#>>'{data,latest_message_id}' is not null
 from chat_results where label='owner_refresh'),'realtime refresh re-authorises and normalises the conversation');
select ok((select exists(select 1 from audit.audit_logs
 where actor_kind='superadmin_internal' and action_code='chat.message.send'
   and actor_internal_identity_id='9c100000-0000-4000-8000-000000000301' and outcome='success')),
 'send is audited with the 13-argument internal audit function');
...
select ok(has_function_privilege('authenticated','public.superadmin_chat_inbox_v2(timestamptz,uuid,integer,text,boolean)','execute')
 ...
 and not has_function_privilege('public','public.superadmin_chat_send_message_v2(uuid,text,uuid)','execute'),
