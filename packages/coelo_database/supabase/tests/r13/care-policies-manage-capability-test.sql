...
-- H17: capacidade propria, sem papel fixo. Unidade B1 para o operations escopado a B.
insert into public.units(id,institution_id,unit_type_id,name,slug,status,handle) values
 ('9f080000-0000-4000-8000-000000000021','9f080000-0000-4000-8000-000000000020','9f080000-0000-4000-8000-000000000002','Unidade B1','p32-b1','active','u.p32b1');
select ok(exists(select 1 from public.platform_permissions where code='care_policies.manage' and status='active'),'care_policies.manage exists in the superadmin catalog');
select ok(exists(select 1 from public.platform_role_permissions rp join public.platform_roles r on r.id=rp.role_id join public.platform_permissions p on p.id=rp.permission_id where r.code='owner' and p.code='care_policies.manage' and rp.status='active'),'owner holds care_policies.manage');
select ok(exists(select 1 from public.institution_role_permissions rp join public.institution_roles r on r.id=rp.role_id join public.institution_permissions p on p.id=rp.permission_id where r.code='institution_admin' and r.institution_id is null and p.code='care_policies.manage' and rp.status='active'),'institution_admin system profile holds care_policies.manage');
...
insert into pol values('owner_set',public.superadmin_unit_care_policy_set_v1('9f080000-0000-4000-8000-000000000931','9f080000-0000-4000-8000-000000000011','{"child_safety_mode":"inclusion_only"}'));
select is((select body->>'ok' from pol where label='owner_set'),'true','owner sets policy by capability');
...
insert into pol values('ops_set',public.superadmin_unit_care_policy_set_v1('9f080000-0000-4000-8000-000000000932','9f080000-0000-4000-8000-000000000021','{"child_safety_mode":"inclusion_only"}'));
select is((select body#>>'{error,code}' from pol where label='ops_set'),'SAI_PERMISSION_DENIED','role without care_policies.manage is denied even with units.update');
reset role;
insert into public.platform_role_permissions(role_id,permission_id,effect,status)
select r.id,p.id,'allow','active' from public.platform_roles r, public.platform_permissions p where r.code='operations' and p.code='care_policies.manage'
on conflict(role_id,permission_id) do update set effect='allow',status='active',revoked_at=null;
set local role authenticated;
insert into pol values('ops_set2',public.superadmin_unit_care_policy_set_v1('9f080000-0000-4000-8000-000000000933','9f080000-0000-4000-8000-000000000021','{"child_safety_mode":"inclusion_only"}'));
select is((select body->>'ok' from pol where label='ops_set2'),'true','granting the capability alone enables the write in scope');
