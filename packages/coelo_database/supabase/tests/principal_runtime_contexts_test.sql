...
select ok(
  pg_get_function_result('public.list_my_principal_contexts()'::regprocedure) =
    'TABLE(membership_id uuid, person_id uuid, institution_id uuid, institution_name text, role_code text, scope_kind text, unit_id uuid, unit_name text, group_id uuid, group_name text, institution_handle text, unit_handle text, access_blocked boolean, access_reason text, access_popup jsonb)',
  'resolver exposes only the minimum runtime-context projection (institution_handle e unit_handle desde 20260911130100; access_* desde 20260919100000)'
);
select ok(
