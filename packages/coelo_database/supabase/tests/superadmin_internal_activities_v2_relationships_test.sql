...
insert into activity_v2_rel_result values('professional_b',public.superadmin_activity_set_professionals_v2('73000000-0000-4000-8000-000000000096',(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create'),4,jsonb_build_array(jsonb_build_object('membership_id','73000000-0000-4000-8000-000000000042','role','instructor','group_id','73000000-0000-4000-8000-000000000021'))));
...
insert into activity_v2_rel_result values('clear',public.superadmin_activity_set_professionals_v2('73000000-0000-4000-8000-000000000097',(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create'),4,'[]'));
...
insert into activity_v2_rel_result values('reactivate',public.superadmin_activity_set_professionals_v2('73000000-0000-4000-8000-000000000098',(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create'),5,jsonb_build_array(jsonb_build_object('membership_id','73000000-0000-4000-8000-000000000041','role','instructor','group_id','73000000-0000-4000-8000-000000000021'))));
...
select ok((select count(*)=2 and count(*) filter(where status='active')=1 and count(*) filter(where revoked_at is not null)=1 from public.activity_group_assignments where membership_id='73000000-0000-4000-8000-000000000041'),'one new active row coexists with one revoked historical row');
...
insert into activity_v2_rel_result values('aal1',public.superadmin_activity_set_participants_v2(gen_random_uuid(),(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create'),6,'[]'));
...
select set_config('request.jwt.claims',jsonb_build_object('sub','73000000-0000-4000-8000-000000000081','session_id','73000000-0000-4000-8000-000000000082','aal','aal2','role','authenticated')::text,true);
insert into activity_v2_rel_result values('expired',public.superadmin_activity_set_participants_v2(gen_random_uuid(),(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create'),6,'[]'));
...
select is((select management_version from public.activity_definitions where id=(select (body#>>'{data,activity_id}')::uuid from activity_v2_rel_result where kind='create')),6::bigint,'rejected commands leave version unchanged');
select * from finish();
rollback;
