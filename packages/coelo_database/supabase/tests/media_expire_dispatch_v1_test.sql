...
select is(app_private.forms_media_dispatch_expire_worker(),null,'sem Vault o dispatch do form-media devolve null');
...
select ok(not has_function_privilege('authenticated','app_private.forms_media_dispatch_expire_worker()','execute')
  and not has_function_privilege('service_role','app_private.forms_media_dispatch_expire_worker()','execute'),'dispatch do form-media sem grant a cliente');
select * from finish();
rollback;
