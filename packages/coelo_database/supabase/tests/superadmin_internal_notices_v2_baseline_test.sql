-- pgTAP de Avisos v2 sobre a baseline (candidato 20260910200000). Derivado de superadmin_internal_notices_v2_test.sql: AAL1 le (MFA fora do MVP) e a constraint nasce validada.
begin;
...
select is((select body#>>'{ok}' from notice_results where label='owner_aal1'),'true',
  'Owner AAL1 reads: MFA fora do MVP (ADR 0034, Decisao 12)');
...
select ok((select convalidated is true from pg_constraint
  where conname='platform_notices_internal_v2_values_ck'),
  'canonical check is validated: production had no rows');
