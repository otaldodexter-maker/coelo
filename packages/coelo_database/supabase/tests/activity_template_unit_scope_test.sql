...
select ok(exists(select 1 from pg_constraint where conname='activity_templates_scope_kind_check' and contype='c' and conrelid='public.activity_templates'::regclass),'scope values are constrained');
select ok(exists(select 1 from pg_constraint where conname='activity_templates_scope_hierarchy_check' and contype='c' and conrelid='public.activity_templates'::regclass),'scope hierarchy is constrained');
