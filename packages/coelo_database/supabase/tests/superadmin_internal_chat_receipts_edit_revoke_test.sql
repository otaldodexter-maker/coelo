...
select has_table('app_private','superadmin_internal_chat_message_edits');
select is((select requires_mfa from public.platform_permissions where code='chat.internal.manage'),
 true,'managing own internal messages requires aal2');
...
insert into auth.users(id,aud,role,email,email_confirmed_at,created_at,updated_at,raw_app_meta_data,raw_user_meta_data) values
 ...
 ('9c110000-0000-4000-8000-000000000103','authenticated','authenticated','manage-owner-aal1@invalid.test',now(),now(),now(),'{}','{}'),
...
insert into auth.sessions(id,user_id,created_at,updated_at,aal,not_after) values
 ('9c110000-0000-4000-8000-000000000201','9c110000-0000-4000-8000-000000000101',now(),now(),'aal2',now()+interval '1 hour'),
 ...
 ('9c110000-0000-4000-8000-000000000206','9c110000-0000-4000-8000-000000000106',now(),now(),'aal2',now()+interval '1 hour');
...
-- Owner de plataforma com aal2: le a thread, edita, repete a edicao e revoga a propria mensagem.
select set_config('request.jwt.claims',jsonb_build_object('sub','9c110000-0000-4000-8000-000000000101','session_id','9c110000-0000-4000-8000-000000000201','aal','aal2','role','authenticated')::text,true);
...
-- Identidade sem chat.internal.manage e owner sem segundo fator nao alteram nada.
...
insert into chat_results values('edit_denied_aal1',
 public.superadmin_chat_edit_message_v2('9c110000-0000-4000-8000-000000000701',
  '9c110000-0000-4000-8000-000000000810','Sem segundo fator','9c110000-0000-4000-8000-000000000907'));
...
select set_config('request.jwt.claims',jsonb_build_object('sub','9c110000-0000-4000-8000-000000000106','session_id','9c110000-0000-4000-8000-000000000206','aal','aal2','role','authenticated')::text,true);
...
select ok((select body#>>'{ok}'='true' and body#>>'{data,body_text}'='Aviso interno corrigido'
  ...
  from chat_results where label='edit_ok'),'author edits own message with aal2');
...
-- Capacidade dedicada e segundo fator.
...
select is((select body#>>'{error,code}' from chat_results where label='edit_denied_aal1'),
 'SAI_MFA_REQUIRED','owner without aal2 cannot edit');
select is((select count(*) from app_private.superadmin_internal_chat_message_edits
  where new_body_text in('Sem capacidade','Sem segundo fator','Tentativa de terceiro','Tarde demais')),
...
select ok((select body#>>'{ok}'='true' and body#>>'{data,revoked_at}' is not null
 ...
 'author revokes own message with aal2');
