-- Circulares (Superadmin) v2 sobre a baseline de producao (ADR 0034, Decisao 8/P12).
-- Reentrega de 20260901191921_superadmin_internal_circulars_v2 (0/14 objetos em
-- producao), corrigida para a forma canonica:
...
--   * preflight recusa replay quando o gateway v2 ja existe (forward-only);
--   * circular_audience_rules.revision_id e NOT NULL em producao: o save
--     grava a regra na revisao de trabalho, como o save_circular_draft;
--   * audit_logs.reason_code exige codigo maiusculo: o helper de auditoria
--     normaliza o motivo (SAVED, PUBLISHED, CLOSED).
-- As RPCs people-based (publish_circular, close_circular_responses etc.) seguem
-- independentes para o Principal; esta superficie nunca cria public.people
-- para um funcionario interno.
...
begin
  if current_user <> 'postgres'
    ...
    or to_regclass('public.circular_revisions') is null
    or to_regclass('app_private.circular_audit') is null
  ...
  if to_regprocedure('public.superadmin_circular_directory_v2(uuid,text,text[],timestamptz,uuid,integer)') is not null
    or to_regprocedure('app_private.superadmin_circular_context(text,uuid)') is not null then
    raise object_not_in_prerequisite_state using
      message = 'superadmin circulars v2 already present; forward-only package must not be replayed';
  end if;
end
$preflight$;
...
begin
  perform app_private.audit_append_superadmin_internal(
    ...
    nullif(upper(regexp_replace(left(p_reason,79),'[^A-Za-z0-9_]','_','g')),''),
    p_correlation,p_ctx.resolved_institution_id,'circular',p_resource);
end
$$;
