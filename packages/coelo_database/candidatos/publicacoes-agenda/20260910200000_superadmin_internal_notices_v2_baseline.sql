-- Avisos (Superadmin) v2 sobre a baseline de producao (ADR 0034, Decisao 8/P12).
-- Reentrega de 20260901185008_superadmin_internal_notices_v2 (0/16 objetos em
-- producao alem de analytics.notice_events), corrigida para a forma canonica:
--   * platform_permissions exige module_label/screen_label/action_label;
--   * requires_mfa=false em todas as capacidades (ADR 0034, Decisao 12);
--   * public.notice_events nao existe em producao: a tabela canonica e
--     analytics.notice_events (ja presente); nada e migrado dela;
--   * o indice de ciclo de vida deixa de ser parcial para nao usar label novo
--     do enum na mesma transacao (ALTER TYPE ... ADD VALUE fica fora do begin);
--   * a check constraint nasce validada: producao tem zero linhas;
--   * superadmin_notice_directory_v2: o CTE de pagina nao convertia para
--     platform_notices (coluna page_row extra) e o record next_row nao
--     atribuido derrubava a funcao sem proxima pagina; corrigidos;
--   * superadmin_notice_change_status_v2: audit_logs.reason_code exige codigo
--     (^[A-Z][A-Z0-9_]*$); o motivo livre de inativacao passa a ficar em
--     silencing_policy.inactive_reason e a auditoria recebe um codigo fixo.
-- A cadeia legada people-based (20260812003000, 20260820204824, 20260820212340,
-- 20260820220500) NAO e chamada pelo app e fica fora deste pacote; o worker de
-- materializacao de audiencia pertence a entrega ao Principal (pos-MVP).
...
begin
  if current_user <> 'postgres'
    ...
    raise object_not_in_prerequisite_state using
      message = 'superadmin internal notices v2 prerequisites missing';
  end if;
  if to_regprocedure('public.superadmin_notice_directory_v2(text[],text,text[],text[],timestamptz,uuid,integer)') is not null then
    raise object_not_in_prerequisite_state using
      message = 'superadmin notices v2 already present; forward-only package must not be replayed';
  end if;
end
$preflight$;
...
revoke all on table public.platform_notices, public.notice_rules, public.notice_media,
  public.notice_receipts, app_private.superadmin_notice_command_receipts
from public, anon, authenticated, service_role;

-- analytics.notice_events ja e a tabela canonica em producao; aqui so endurece.
...
declare context_record app_private.superadmin_internal_context;
  correlation_id uuid := gen_random_uuid(); items jsonb;
  next_updated_at timestamptz; next_id uuid;
...
begin
  begin
    ...
    with page as (
      select notice_record notice, notice_record.updated_at, notice_record.id,
    ...
    select coalesce(jsonb_agg(app_private.superadmin_notice_json(page.notice)
      order by page.updated_at desc, page.id desc) filter(where page.page_row <= p_limit), '[]'::jsonb),
      count(*) > p_limit
    into items, has_more from page;
    if has_more then
      select updated_at, id into next_updated_at, next_id from public.platform_notices notice_record
      where (p_search is null or notice_record.title ilike '%' ||
          replace(replace(btrim(p_search), '%', '\%'), '_', '\_') || '%' escape '\')
        and (p_types is null or notice_record.notice_type::text = any(p_types))
        and (p_statuses is null or notice_record.status::text = any(p_statuses))
    ...
    return jsonb_build_object('ok', true, 'data', jsonb_build_object(
      'items', items,
      'next_cursor_occurred_at', case when has_more then next_updated_at else null end,
      'next_cursor_id', case when has_more then next_id else null end), 'error', null);
