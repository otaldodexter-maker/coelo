...
| --- | --- | --- | --- |
| 1 | `20260911130400_internal_actor_institution_access_by_role_v1.sql` | P48 = A: papel de sistema `institution_reader` (Leitura, so `*.read`) e sincronizador `superadmin_internal_actor_institution_access_sync()` por papel interno: owner -> membership `owner` + `institution_admin`; operations -> membership `professional` + `institution_reader`; demais papeis internos sem vinculo automatico (o que o 130000 tinha concedido e revogado: `inactive` + `revoked_at`). Backfill na aplicacao; gatilhos do 130000 preservados. | REESCRITO 21:30 sobre o corpo do `20260912210100` (escopo por `superadmin_internal_actor_scope_targets()`, reconciliacao fora do escopo preservada), conforme a retencao do coordenador. `internal_actor_scope_root_v1_test` 46/46, `internal_actor_scope_root_v1_hotfix_test` 4/4, `internal_actor_institution_access_by_role_v1_test` 16/16, `principal_internal_actor_bridge_v1_test` 22/22 no clone do espelho (lotes 50+52 incluidos) |
