...
-- interna e persistidos.
--
-- Fixar conversas e os sinalizadores de bandeira existiam na tela e sumiram
-- quando 2ffc126db trocou a inbox de fixture por dados reais; o Owner
-- classificou a perda como regressao ao decidir os goldens do Chat em
-- 10/09/2026. O estado anterior vivia so na memoria do controller
-- (_pinnedOrder e _flags), entao restaura-lo do jeito antigo reintroduziria
-- algo que o reload apaga. Aqui a preferencia passa a ser do banco.
--
-- A preferencia pertence a identidade interna, nao a conversa: duas pessoas na
-- mesma conversa fixam e sinalizam de forma independente. A tabela fica em
-- app_private, como os demais recibos internos, e so e alcancada pelas RPCs
-- v2, que revalidam contexto, capacidade e escopo de instituicao.
--
-- Criar grupo continua fora deste pacote: depende de criar conversa e membros,
-- que e um pacote proprio.
begin;
...
begin
  begin
    ...
    perform app_private.audit_append_superadmin_internal(
      ...
      jsonb_build_object('pinned', saved.pinned_at is not null, 'flag', saved.flag));
