...
function sessionClaims(token) {
  try {
    ...
    return {sub: claims.sub, session_id: claims.session_id, aal: claims.aal, role: 'authenticated', exp: claims.exp};
...
export function mutationSql(action, mailbox, actor) {
  preview(action);
  if (!uuid.test(actor.sub) || !uuid.test(actor.session_id) || !['aal1', 'aal2'].includes(actor.aal) ||
      !Number.isFinite(actor.exp) || actor.exp <= Date.now()/1000) fail('LIVE_SESSION_REQUIRED');
  const claims = {sub: actor.sub, session_id: actor.session_id, aal: actor.aal, role: 'authenticated', exp: actor.exp};
  // The canonical helper uses transaction-time now(). Recheck actual expiry
  // after waits and before each effect without changing that shared helper.
  const ownerGuard = `
  ...
  if ctx.platform_role_code is distinct from 'owner' or ctx.scope_kind is distinct from 'platform'
    or ctx.auth_user_id is distinct from '${actor.sub}'::uuid or ctx.session_id is distinct from '${actor.session_id}'::uuid
    or extract(epoch from clock_timestamp()) >= ${actor.exp}
    or not exists(select 1 from auth.sessions s where s.id='${actor.session_id}'::uuid
      and s.user_id='${actor.sub}'::uuid and (s.not_after is null or s.not_after > clock_timestamp()))
    then raise exception 'LIVE_OWNER_REQUIRED'; end if;`;
  const provision = `
...
export async function execute(action, input, fetcher = fetch) {
  ...
  sessionClaims(input.actorToken);
...
select jsonb_build_object('authorized',ctx.platform_role_code='owner' and ctx.scope_kind='platform'
  and ctx.auth_user_id='${actor.sub}'::uuid and ctx.session_id='${actor.session_id}'::uuid
  and extract(epoch from clock_timestamp()) < ${actor.exp}
  and exists(select 1 from auth.sessions s where s.id='${actor.session_id}'::uuid
    and s.user_id='${actor.sub}'::uuid and (s.not_after is null or s.not_after > clock_timestamp()))) as actor
...
commit;`,'OWNER_CONTEXT_UNCONFIRMED');
  ...
  if (action === 'provision') {
    ...
    if (current.banned) {
      current = await readState();
      if (!current.owned || !current.no_global_link || !current.role_ready || current.mailbox_collisions ||
          current.bindings !== 'active' || !current.banned || current.sessions !== 0) fail('BINDINGS_UNCONFIRMED');
      ownedUser(await auth(`/admin/users/${plan.authUserId}`), input.mailbox);
      await assertLiveOwner();
    ...
    if (!(current.banned && current.sessions===0 && ['absent','revoked'].includes(current.bindings))) {
      // Authorization denial must never be swallowed as an ambiguous ban.
      await assertLiveOwner();
      try {
