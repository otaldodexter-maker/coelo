...
function Start-Psql([string]$Sql,[switch]$KeepOpen) {
  ...
  $info.Arguments="exec -i $containerName psql --no-psqlrc --set ON_ERROR_STOP=1 --tuples-only --no-align --username postgres --dbname postgres"
...
function Finish-Psql([Diagnostics.Process]$Process,[switch]$ExpectDenial) {
  ...
  if ($ExpectDenial) {
    if ($Process.ExitCode -eq 0 -or $stderr -notmatch 'LIVE_OWNER_REQUIRED|SAI_MEMBERSHIP_REVOKED|SAI_SESSION_INVALID' -or
        $stderr -match 'lock timeout|deadlock|statement timeout') { throw 'AUTH_PROOF_EXPECTED_DENIAL_NOT_OBSERVED' }
  } elseif ($Process.ExitCode -ne 0) { throw 'AUTH_PROOF_LOCAL_SQL_FAILED' }
  return $stdout
}
...
select v.id::uuid,v.identity::uuid,r.id,'platform' from public.platform_roles r cross join (values
 ('$membership','$identity'),('a9020000-0000-4000-8000-000000000502','a9020000-0000-4000-8000-000000000302')) v(id,identity) where r.code='owner';
commit;
"@)
  $fixtureReady=$true
...
"@)
    ...
    foreach ($scenario in @('membership','session','jwt')) {
      $case="$action-$scenario"; $holderName="r02_auth_holder_$scenario"; $callerName="r02_auth_caller_$scenario"
      $null=Finish-Psql (Start-Psql "update app_private.superadmin_internal_memberships set status='active',revoked_at=null where id='$membership'; update auth.sessions set not_after=clock_timestamp()+interval '1 hour' where id='$session';")
