...
test('opaque secret key uses apikey only while actor JWT stays in Authorization',async()=>{
  ...
  assert.ok(adminRequests>0);assert.ok(actorRequests>=4);
});
...
test('mutation SQL enforces live Owner, ownership, cross-realm, exact versions and audit within transaction',()=>{
  const actor={sub:actorId,session_id:sessionId,aal:'aal1',exp:Date.now()/1000+3600};
...
for (const phase of ['before-create', 'before-bindings', 'before-unban', 'before-ban', 'before-cleanup']) {
  test(`Owner revoked ${phase} denies the next privileged mutation`, async () => {
    const cleanup = phase === 'before-ban' || phase === 'before-cleanup';
    const s = server(cleanup ? {auth:true, bindings:'active', banned:false, sessions:1} : {});
    let fired = false;
    const fetcher = async (url, options) => {
      const response = await s.fetcher(url, options);
      const query = options.body ? JSON.parse(options.body).query ?? '' : '';
      const trigger = phase === 'before-create' ? query.startsWith('begin read only;\nselect jsonb_build_object')
        : phase === 'before-bindings' ? url.endsWith('/admin/users') && options.method === 'POST'
        : phase === 'before-unban' ? query.includes('e2.r02.auth.provision')
        : phase === 'before-ban' ? url.endsWith(`/admin/users/${plan.authUserId}`) && options.method === 'GET'
        : url.endsWith(`/admin/users/${plan.authUserId}`) && options.method === 'PUT';
      if (!fired && trigger) { fired = true; s.state.actor = false; }
      return response;
    };
    await assert.rejects(execute(cleanup ? 'cleanup' : 'provision', input(cleanup ? 'cleanup' : 'provision'), fetcher), /LIVE_OWNER_REQUIRED/);
    assert.equal(fired, true);
    assert.equal(s.mutations().length, {'before-create':0, 'before-bindings':1, 'before-unban':2, 'before-ban':0, 'before-cleanup':1}[phase]);
  });
}

for (const delta of [{bindings:'revoked'}, {bindings:'drift'}, {global:true}, {role:false}, {owned:false}, {collisions:1}, {sessions:1}]) {
  test(`fresh state prevents unban after drift ${JSON.stringify(delta)}`, async () => {
    const s = server();
    const fetcher = async (url, options) => {
      const response = await s.fetcher(url, options);
      if (options.body && (JSON.parse(options.body).query ?? '').includes('e2.r02.auth.provision')) Object.assign(s.state, delta);
      return response;
    };
    await assert.rejects(execute('provision',input('provision'),fetcher), /BINDINGS_UNCONFIRMED/);
    assert.equal(s.mutations().length,2);
    assert.equal(s.state.banned,true);
  });
}

test('JWT expiring after initial state prevents Auth create', async () => {
  const s=server(); const data=input('provision'); const realNow=Date.now;
  const fetcher=async(url,options)=>{
    const response=await s.fetcher(url,options);
    if(options.body && (JSON.parse(options.body).query ?? '').startsWith('begin read only;\nselect jsonb_build_object')) Date.now=()=>realNow()+7200000;
    return response;
  };
  try {
    await assert.rejects(execute('provision',data,fetcher),/LIVE_SESSION_REQUIRED/);
    assert.equal(s.mutations().length,0);
  } finally { Date.now=realNow; }
});

test('SQL repeats current-clock Owner guards after locks and before each privileged DML', () => {
  for (const action of ['provision','cleanup']) {
    const sql=mutationSql(action,input(action).mailbox,{sub:actorId,session_id:sessionId,aal:'aal1',exp:2000000000});
    const lastLock=sql.lastIndexOf('for update;');
    const writes=[...sql.matchAll(/\n  (?:insert into|update app_private|delete from auth.sessions)/g)];
    assert.equal(writes.length,3);
    let previous=lastLock;
    for(const write of writes) {
      const guard=sql.slice(previous,write.index);
      assert.match(guard,/into strict ctx/);
      assert.match(guard,/not_after > clock_timestamp\(\)/);
      assert.match(guard,/extract\(epoch from clock_timestamp\(\)\) >= 2000000000/);
      previous=write.index;
    }
  }
  assert.throws(()=>mutationSql('provision',input('provision').mailbox,{sub:actorId,session_id:sessionId,aal:'aal1'}),/LIVE_SESSION_REQUIRED/);
});
