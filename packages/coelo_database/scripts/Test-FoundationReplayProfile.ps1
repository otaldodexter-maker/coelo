...
if ($entries.Count -ne 67) {
  throw "foundation replay profile must contain exactly 67 canonical migrations; found $($entries.Count)"
}
if ($entries[0].Name -ne '20260623191021_superadmin_foundation_v1.sql' -or
    $entries[-1].Name -ne '20260901200206_defer_superadmin_internal_mfa_until_mvp_go_live.sql') {
...
"Foundation replay profile PASS: 67 reviewed canonical migrations; eight product migrations denied; all SHA-256 values match."
