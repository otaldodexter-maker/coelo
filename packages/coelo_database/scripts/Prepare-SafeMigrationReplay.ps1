...
if ($NominalProfile) {
  $nominalResolverRelative = switch ($NominalProfile) {
    ...
    'ModelAal1PhasePolicy' { 'profiles\ModelAal1PhasePolicy\Resolve-ModelAal1PhasePolicy.ps1' }
  ...
  if ($NominalProfile -ceq 'LocationReservationsV1') {
    $locationBootstraps = @($nominal.LocationBootstrap)
    if ($canonical.Count -ne 59 -or $additionalCanonical.Count -ne 6 -or
