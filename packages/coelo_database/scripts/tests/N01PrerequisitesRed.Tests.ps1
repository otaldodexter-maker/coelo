...
function Get-N01TestHash([string]$Path, [switch]$Raw) {
...
Describe 'Closed N01PrerequisitesRed replay selector' {
  BeforeEach {
    ...
    $descriptorPath = Join-Path $fixtureReplayRoot 'profiles\N01PrerequisitesRed\profile.json'
    ...
    [IO.File]::WriteAllText($invokeScript, $invokeText.Replace(
      $sentinelAnchor, "throw 'N01 fixture stop before mutex or Docker'`n" + $sentinelAnchor))
    $configRoot = Join-Path $fixturePackageRoot 'supabase'
    New-Item -ItemType Directory -Path $configRoot | Out-Null
    [IO.File]::WriteAllText((Join-Path $configRoot 'config.toml'), 'project_id = "n01_fixture"')
    $nominalNames = @(
      '20260812002900_notices_status_values.sql',
  ...
  It 'selects Auth45 plus exactly five historical files and two preflights with identical bytes' {
    $output = & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile N01PrerequisitesRed
    ...
    foreach ($file in $actual) {
      ...
      (Get-N01TestHash $file.FullName -Raw) | Should Be (Get-N01TestHash (Join-Path $root $file.Name) -Raw)
      (Get-N01TestHash $file.FullName) | Should Be (Get-N01TestHash (Join-Path $root $file.Name))
    }
    # Notices v2 belongs to Foundation67, but is not already in Auth45.
    ($baselineNames -contains $nominalNames[-1]) | Should Be $false
    @($actual | Where-Object Name -eq $nominalNames[-1]).Count | Should Be 1
    @($actual.Name | ForEach-Object { $_.Substring(0, 14) } | Sort-Object -Unique).Count | Should Be 52
    $actual[-1].Name.Substring(0, 14) | Should Be '20260901200206'
    $output | Should Match 'profile=N01PrerequisitesRed; additional=5'
  }

  It 'accepts the sole nominal target and reaches only the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01 fixture stop before mutex or Docker'
  }

  It 'rejects an earlier target before the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901185008 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed requires target 20260901200206'
  }

  It 'rejects a non-allowlisted profile' {
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile GreenN01 } |
      Should Throw 'ValidateSet'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile GreenN01 } |
      Should Throw 'ValidateSet'
  }
  ...
  It 'rejects CLI profile mixing: <mode>' -TestCases @(
    ...
    param($mode)
    $parameters = @{ NominalProfile = 'N01PrerequisitesRed' }
  ...
  It 'rejects descriptor mutation <field> before copying or the Invoke sentinel' -TestCases @(
    ...
    switch ($field) {
      ...
      name { $descriptor.canonical_additions[0].file = '20260812002900_wrong.sql' }
      hash { $descriptor.canonical_additions[0].sha256_crlf_utf8 = '0' * 64 }
      target { $descriptor.target_version = '20260901185008' }
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed descriptor hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed descriptor hash mismatch'
  ...
  It 'rejects a changed foundation manifest before either entry point can proceed' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed base manifest hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed base manifest hash mismatch'
  }
  ...
  It 'rejects changed SQL bytes for <inputKind> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed input hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed input hash mismatch'
    @(Get-ChildItem -LiteralPath $destination -Force).Count | Should Be 0
  }
  ...
  It 'rejects a missing canonical addition' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed input is missing'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile N01PrerequisitesRed } |
      Should Throw 'N01PrerequisitesRed input is missing'
  }
  ...
  It 'rejects a resolver <kind> reparse point in <entrypoint> before executing it' -TestCases @(
    ...
    $resolverPath = Join-Path $fixtureReplayRoot 'profiles\N01PrerequisitesRed\Resolve-N01PrerequisitesRed.ps1'
