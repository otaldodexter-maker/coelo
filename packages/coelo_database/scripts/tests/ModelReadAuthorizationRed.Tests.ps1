...
function Get-ModelReadTestHash([string]$Path, [switch]$Raw) {
...
Describe 'Closed ModelReadAuthorizationRed replay selector' {
  BeforeEach {
    ...
    $descriptorPath = Join-Path $fixtureReplayRoot 'profiles\ModelReadAuthorizationRed\profile.json'
    ...
    [IO.File]::WriteAllText($invokeScript, $invokeText.Replace(
      $sentinelAnchor, "throw 'ModelRead fixture stop before mutex or Docker'`n" + $sentinelAnchor))
    $configRoot = Join-Path $fixturePackageRoot 'supabase'
    New-Item -ItemType Directory -Path $configRoot | Out-Null
    [IO.File]::WriteAllText((Join-Path $configRoot 'config.toml'), 'project_id = "model_read_fixture"')
    $nominalNames = @(
  ...
  It 'selects Auth45 plus exactly two historical files and two preflights with identical bytes' {
    $output = & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationRed
    ...
    foreach ($file in $actual) {
      ...
      (Get-ModelReadTestHash $file.FullName -Raw) | Should Be (Get-ModelReadTestHash (Join-Path $root $file.Name) -Raw)
      (Get-ModelReadTestHash $file.FullName) | Should Be (Get-ModelReadTestHash (Join-Path $root $file.Name))
    }
    # These two historical prerequisites must appear once at the approved canonical positions.
    ($baselineNames -contains $nominalNames[-1]) | Should Be $false
    @($actual | Where-Object Name -eq $nominalNames[-1]).Count | Should Be 1
    @($actual.Name | ForEach-Object { $_.Substring(0, 14) } | Sort-Object -Unique).Count | Should Be 49
    $actualCanonical = @($actual | Where-Object { $_.Name -notin $preflightNames })
    $actualCanonical.Count | Should Be 47
    for ($additionIndex = 0; $additionIndex -lt $nominalNames.Count; $additionIndex++) {
      ([Array]::IndexOf(@($actualCanonical.Name), $nominalNames[$additionIndex]) + 1) | Should Be (@(45, 46)[$additionIndex])
    }
    $actual[-1].Name.Substring(0, 14) | Should Be '20260901200206'
    $output | Should Match 'profile=ModelReadAuthorizationRed; additional=2'
  }

  It 'accepts the sole nominal target and reaches only the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelRead fixture stop before mutex or Docker'
  }

  It 'rejects an earlier target before the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901170731 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed requires target 20260901200206'
  }

  It 'rejects a non-allowlisted profile' {
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile GreenModelRead } |
      Should Throw 'ValidateSet'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile GreenModelRead } |
      Should Throw 'ValidateSet'
  }
  ...
  It 'rejects CLI profile mixing: <mode>' -TestCases @(
    ...
    param($mode)
    $parameters = @{ NominalProfile = 'ModelReadAuthorizationRed' }
  ...
  It 'rejects descriptor mutation <field> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed descriptor hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed descriptor hash mismatch'
  ...
  It 'rejects a changed foundation manifest before either entry point can proceed' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed base manifest hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed base manifest hash mismatch'
  }
  ...
  It 'rejects changed SQL bytes for <inputKind> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed input hash mismatch'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed input hash mismatch'
    @(Get-ChildItem -LiteralPath $destination -Force).Count | Should Be 0
  }
  ...
  It 'rejects a missing canonical addition' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed input is missing'
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationRed } |
      Should Throw 'ModelReadAuthorizationRed input is missing'
  }
  ...
  It 'rejects a resolver <kind> reparse point in <entrypoint> before executing it' -TestCases @(
    ...
    $resolverPath = Join-Path $fixtureReplayRoot 'profiles\ModelReadAuthorizationRed\Resolve-ModelReadAuthorizationRed.ps1'
