...
function Get-FReadDerivedTestHash([string]$Path, [switch]$Raw) {
...
function New-FReadDerivedFixture([string]$Root) {
  ...
  [IO.File]::WriteAllText($invoke, $invokeText.Replace(
    $anchor, "throw 'FRead derived fixture stop before mutex or Docker'`n" + $anchor))
  ...
  $profile = Join-Path $replay 'profiles\FReadDirectoryContractRedDerived'
  [pscustomobject]@{
    ...
    Resolver = Join-Path $profile 'Resolve-FReadDirectoryContractRedDerived.ps1'
    Descriptor = Join-Path $profile 'profile.json'
    Converter = Join-Path $profile 'Convert-FReadFormsDefinitionForLocalReplay.ps1'
    ParentResolver = Join-Path $replay 'profiles\FReadDirectoryContractRed\Resolve-FReadDirectoryContractRed.ps1'
    ParentDescriptor = Join-Path $replay 'profiles\FReadDirectoryContractRed\profile.json'
    Source = Join-Path $migrations '20260813155005_forms_definition_and_capabilities.sql'
  }
}

Describe 'Closed FRead derived resolver' -Tag 'FReadDerivedResolver' {
  BeforeEach { $fixture = New-FReadDerivedFixture (Join-Path $TestDrive ([guid]::NewGuid().ToString('N'))) }

  It 'returns the unchanged RED50 inputs and one pinned materialization without writing or executing it' {
    $sourceBefore = Get-FReadDerivedTestHash $fixture.Source -Raw
    $parent = & $fixture.ParentResolver
    $result = & $fixture.Resolver
    $result.Canonical.Count | Should Be 48
    $result.Preflight.Count | Should Be 2
    $result.Additional.Count | Should Be 3
    ...
    (Get-FReadDerivedTestHash $fixture.Source -Raw) | Should Be $sourceBefore
    @(Get-ChildItem -LiteralPath $fixture.Destination -Force).Count | Should Be 0
  }

  It 'rejects a target beyond the fixed original RED boundary' {
    { & $fixture.Resolver -TargetVersion 20260908000049 } |
      Should Throw 'FReadDirectoryContractRedDerived requires target 20260901200206'
  }
  ...
  It 'rejects descriptor drift in <field>' -TestCases @(
    ...
    switch ($field) {
      ...
      target { $descriptor.target_version = '20260908000049' }
      bridge { $descriptor.extra_bridges = @('unapproved.sql') }
      count { $descriptor.planned_counts.total = 51 }
    }
    [IO.File]::WriteAllText($fixture.Descriptor, ($descriptor | ConvertTo-Json -Depth 10))
    { & $fixture.Resolver } | Should Throw 'FReadDirectoryContractRedDerived descriptor hash mismatch'
  }
  ...
  It 'rejects changed <dependency> before calling inherited code' -TestCases @(
    ...
    { & $fixture.Resolver } | Should Throw 'FReadDirectoryContractRedDerived dependency hash mismatch'
  }
  ...
  It 'retains the canonical input hash gate for <kind>' -TestCases @(
    ...
    { & $fixture.Resolver } | Should Throw 'FReadDirectoryContractRed input hash mismatch'
  }
...
Describe 'Closed FRead derived entrypoints' -Tag 'FReadDerivedEntrypoints' {
  BeforeEach { $fixture = New-FReadDerivedFixture (Join-Path $TestDrive ([guid]::NewGuid().ToString('N'))) }

  It 'materializes the same 50 names with only four inserted bytes in the single approved file' {
    $parent = & $fixture.ParentResolver
    $sourceBefore = Get-FReadDerivedTestHash $fixture.Source -Raw
    & $fixture.Prepare -DestinationMigrationsRoot $fixture.Destination -NominalProfile FReadDirectoryContractRedDerived
    $expected = @(@($parent.Canonical) + @($parent.Preflight) | Sort-Object Name)
    ...
    for ($index = 0; $index -lt $actual.Count; $index++) {
      if ($actual[$index].Name -eq '20260813155005_forms_definition_and_capabilities.sql') {
        (Get-FReadDerivedTestHash $actual[$index].FullName) | Should Be '06b71570bbb25c84efe5efed6a6d1f2416a33a5a6fdf71bc20b4776d01833dfe'
        ($actual[$index].Length - $expected[$index].Length) | Should Be 4
      } else {
        (Get-FReadDerivedTestHash $actual[$index].FullName -Raw) | Should Be (Get-FReadDerivedTestHash $expected[$index].FullName -Raw)
      }
    }
    (Get-FReadDerivedTestHash $fixture.Source -Raw) | Should Be $sourceBefore
  }
  ...
  It 'accepts only the reviewed target and stops before any Docker or staging action' {
    { & $fixture.Invoke -TargetVersion 20260901200206 -NominalProfile FReadDirectoryContractRedDerived } |
      Should Throw 'FRead derived fixture stop before mutex or Docker'
    { & $fixture.Invoke -TargetVersion 20260827235500 -NominalProfile FReadDirectoryContractRedDerived } |
      Should Throw 'FReadDirectoryContractRedDerived requires target 20260901200206'
  }
  ...
  It 'rejects nominal CLI mixing: <mode>' -TestCases @(
    ...
    param($mode)
    $parameters = @{ NominalProfile = 'FReadDirectoryContractRedDerived' }
    ...
    { & $fixture.Invoke -TargetVersion 20260901200206 @parameters } |
      Should Throw 'nominal replay cannot be combined'
  }
  ...
  It 'rejects converter drift before either entrypoint can copy or reach the sentinel' {
    ...
    { & $fixture.Prepare -DestinationMigrationsRoot $fixture.Destination -NominalProfile FReadDirectoryContractRedDerived } |
      Should Throw 'FReadDirectoryContractRedDerived dependency hash mismatch'
    { & $fixture.Invoke -TargetVersion 20260901200206 -NominalProfile FReadDirectoryContractRedDerived } |
      Should Throw 'FReadDirectoryContractRedDerived dependency hash mismatch'
    @(Get-ChildItem -LiteralPath $fixture.Destination -Force).Count | Should Be 0
  }
  ...
  It 'rejects a relative destination before copying any input' {
    ...
    { & $fixture.Prepare -DestinationMigrationsRoot $relative -NominalProfile FReadDirectoryContractRedDerived } |
  ...
  It 'rejects a converter result with <defect> before copying the other 49 inputs' -TestCases @(
    ...
    $descriptorHashBefore = Get-FReadDerivedTestHash $fixture.Descriptor
    $descriptor = Get-Content -LiteralPath $fixture.Descriptor -Raw | ConvertFrom-Json
    $descriptor.forms_definition_materialization.converter_sha256_crlf_utf8 = Get-FReadDerivedTestHash $fixture.Converter
    [IO.File]::WriteAllText($fixture.Descriptor, ($descriptor | ConvertTo-Json -Depth 10))
