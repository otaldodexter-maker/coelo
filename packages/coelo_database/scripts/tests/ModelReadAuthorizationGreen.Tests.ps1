...
Describe 'Closed ModelReadAuthorizationGreen replay selector' {
  BeforeEach {
    ...
    [IO.File]::WriteAllText($invokeScript, $invokeText.Replace(
      $sentinelAnchor, "throw 'Model Green fixture stop before mutex or Docker'`n" + $sentinelAnchor))
  ...
  It 'selects the Model base49 plus only the nominal READ corrective with identical bytes' {
    ...
    foreach ($file in $actual) {
      ...
      (Get-ModelGreenTestHash $file.FullName) | Should Be (Get-ModelGreenTestHash (Join-Path $root $file.Name))
    }
    # Preserve both Models positions and append only the READ corrective after the final Auth MVP migration.
    ...
    for ($additionIndex = 0; $additionIndex -lt $nominalNames.Count; $additionIndex++) {
      ([Array]::IndexOf(@($actualCanonical.Name), $nominalNames[$additionIndex]) + 1) | Should Be (@(45, 46, 48)[$additionIndex])
    }
    ($actual.Name -contains '20260908021821_access_profile_models_read_prelookup_authorization.sql') | Should Be $true
    $actualCanonical[46].Name.Substring(0, 14) | Should Be '20260901200206'
    $actual[-1].Name.Substring(0, 14) | Should Be '20260908021821'
    $output | Should Match 'profile=ModelReadAuthorizationGreen; additional=3'
  }

  It 'accepts the sole nominal target and reaches only the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260908021821 -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'Model Green fixture stop before mutex or Docker'
  }

  It 'rejects an earlier target before the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile ModelReadAuthorizationGreen } |
  ...
  It 'rejects a non-allowlisted profile' {
    ...
    { & $invokeScript -TargetVersion 20260908021821 -NominalProfile UnapprovedModelGreen } |
      Should Throw 'ValidateSet'
  }
  ...
  It 'rejects CLI profile mixing: <mode>' -TestCases @(
    ...
    param($mode)
    $parameters = @{ NominalProfile = 'ModelReadAuthorizationGreen' }
    ...
    { & $invokeScript -TargetVersion 20260908021821 @parameters } |
      Should Throw 'nominal replay cannot be combined'
  }
  ...
  It 'rejects descriptor mutation <field> before copying or the Invoke sentinel' -TestCases @(
    ...
    switch ($field) {
      ...
      count { $descriptor.planned_counts.canonical = 47 }
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen descriptor hash mismatch'
    { & $invokeScript -TargetVersion 20260908021821 -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen descriptor hash mismatch'
  ...
  It 'rejects changed SQL bytes for <inputKind> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen input hash mismatch'
    { & $invokeScript -TargetVersion 20260908021821 -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen input hash mismatch'
    @(Get-ChildItem -LiteralPath $destination -Force).Count | Should Be 0
  }
  ...
  It 'rejects a missing canonical addition' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen input is missing'
    { & $invokeScript -TargetVersion 20260908021821 -NominalProfile ModelReadAuthorizationGreen } |
      Should Throw 'ModelReadAuthorizationGreen input is missing'
  }

  It 'rejects omission of the sole READ corrective before copying or the Invoke sentinel' {
  ...
  It 'rejects a resolver <kind> reparse point in <entrypoint> before executing it' -TestCases @(
    ...
    $resolverPath = Join-Path $fixtureReplayRoot 'profiles\ModelReadAuthorizationGreen\Resolve-ModelReadAuthorizationGreen.ps1'
