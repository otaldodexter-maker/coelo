...
function Get-A01AuditTestHash([string]$Path, [switch]$Raw) {
...
Describe 'Closed A01DirectoryAuditRed replay selector' {
  BeforeEach {
    ...
    $descriptorPath = Join-Path $fixtureReplayRoot 'profiles\A01DirectoryAuditRed\profile.json'
    ...
    [IO.File]::WriteAllText($invokeScript, $invokeText.Replace(
      $sentinelAnchor, "throw 'A01 Audit fixture stop before mutex or Docker'`n" + $sentinelAnchor))
  ...
  It 'selects the A01 base54 plus only the nominal v1 corrective with identical bytes' {
    $output = & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile A01DirectoryAuditRed
    ...
    foreach ($file in $actual) {
      ...
      (Get-A01AuditTestHash $file.FullName -Raw) | Should Be (Get-A01AuditTestHash (Join-Path $root $file.Name) -Raw)
      (Get-A01AuditTestHash $file.FullName) | Should Be (Get-A01AuditTestHash (Join-Path $root $file.Name))
    }
    # Preserve the seven Activities positions and append only v1 after the final Auth MVP migration.
    ...
    $output | Should Match 'profile=A01DirectoryAuditRed; additional=8'
  }

  It 'accepts the sole nominal target and reaches only the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01 Audit fixture stop before mutex or Docker'
  }

  It 'rejects an earlier target before the fixture sentinel' {
    { & $invokeScript -TargetVersion 20260901200206 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed requires target 20260907222911'
  }

  It 'rejects a non-allowlisted profile' {
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile GreenA01Audit } |
      Should Throw 'ValidateSet'
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile GreenA01Audit } |
      Should Throw 'ValidateSet'
  }
  ...
  It 'rejects CLI profile mixing: <mode>' -TestCases @(
    ...
    param($mode)
    $parameters = @{ NominalProfile = 'A01DirectoryAuditRed' }
  ...
  It 'rejects descriptor mutation <field> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed descriptor hash mismatch'
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed descriptor hash mismatch'
  ...
  It 'rejects a changed foundation manifest before either entry point can proceed' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed base manifest hash mismatch'
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed base manifest hash mismatch'
  }
  ...
  It 'rejects changed SQL bytes for <inputKind> before copying or the Invoke sentinel' -TestCases @(
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed input hash mismatch'
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed input hash mismatch'
    @(Get-ChildItem -LiteralPath $destination -Force).Count | Should Be 0
  }
  ...
  It 'rejects a missing canonical addition' {
    ...
    { & $prepareScript -DestinationMigrationsRoot $destination -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed input is missing'
    { & $invokeScript -TargetVersion 20260907222911 -NominalProfile A01DirectoryAuditRed } |
      Should Throw 'A01DirectoryAuditRed input is missing'
  }

  It 'rejects omission of the sole v1 corrective before copying or the Invoke sentinel' {
  ...
  It 'rejects a resolver <kind> reparse point in <entrypoint> before executing it' -TestCases @(
    ...
    $resolverPath = Join-Path $fixtureReplayRoot 'profiles\A01DirectoryAuditRed\Resolve-A01DirectoryAuditRed.ps1'
