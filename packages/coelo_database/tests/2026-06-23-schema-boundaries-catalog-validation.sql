...
begin
  ...
  select count(*),
         string_agg(format('%I.%I', table_schema, table_name), ', '
           order by table_schema, table_name)
  into missing_table_count, missing_tables
  from missing;
  ...
  select count(*),
         string_agg(format('%I.%I.%I', table_schema, table_name, column_name), ', '
           order by table_schema, table_name, column_name)
  into missing_column_count, missing_columns
  from missing;

  if missing_table_count > 0 or missing_column_count > 0 then
    raise exception
      'schema catalog incomplete: % missing active tables [%]; % missing active columns [%]',
      missing_table_count,
      coalesce(missing_tables, ''),
