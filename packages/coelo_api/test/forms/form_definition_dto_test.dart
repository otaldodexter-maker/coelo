...
void main() {
  ...
  for (final config in [<String, Object?>{}, <String, Object?>{'min_selections': 2}, <String, Object?>{'max_selections': 50}, <String, Object?>{'min_selections': 2, 'max_selections': 4}]) {
  ...
  for (final key in ['min_selections', 'max_selections']) {
    for (final value in <Object?>['2', 2.5, true, null]) {
      test('rejects malformed selection limit $key=$value', () {
        ...
        expect(() => FormDefinitionDto.fromJson(json).toDomain(), throwsA(isA<WireFormatException>()));
      });
    }
  }
