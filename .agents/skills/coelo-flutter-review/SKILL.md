...
- Mídia no cliente: fotos de entidade por `EntityImageView`/`EntityIdentityHeader`/`PersonAvatarView` sobre `EntityImageCache` (um lote por tipo e frame, bytes pela Edge; `principal: true` usa o leitor do Principal); bytes de qualquer mídia por `lib/shared/data/edge_media_bytes.dart` (`uploadBytesThroughEdge`, `readBytesThroughEdge`, `mediaObjectUrl` = `blob:` no web/`data:` fora, `sniffMediaMimeType`); vídeo inline por `MediaInlineVideo` (URL local ou assinada). Nunca `http.put`/`Image.network` de URL assinada do R2. Prova de upload no driver CDP exige clique real (`Input.dispatchMouseEvent`) no botão do seletor — `tap` do driver não abre `<input type=file>`; se o chooser não abrir, repita o clique (pickFile com retry em Coelo-backups/midia-proof).
- Atividade nova: `superadmin_activity_create_v2` exige categoria, ≥1 unidade e sigla de 1–2
  letras; a sigla vazia nasce do nome (`activityInitialsFor`, no mapeamento do rascunho) — sem
  isso `_supportsAggregateSave` recusava e a tela culpava a conexão. Prévia do @ da unidade usa
  `institution_slug` da RPC (lote 108), nunca o nome público.
