---
source: "docs/design/design-system.md; contratos administrativos preservados de coelo-ui; esclarecimento do Owner em 2026-09-08"
status: "active"
generated_at: "2026-09-08"
---

# Fluxo administrativo Coelo UI


As baselines e regras de composição abaixo se aplicam à família administrativa.
Principal e Site seguem o roteamento acima, mesmo quando hospedados no Superadmin.
...
1. Identificar produto, tela e contexto. Quando a tarefa mencionar popup, modal,
   ...
   obrigatoriamente o [contrato de superfícies e interação](surface-interaction-contracts.md)
   ...
   [baselines visuais aprovadas do Superadmin](approved-superadmin-visual-baselines.md)
   ...
   [padrões visuais rejeitados](rejected-visual-patterns-inbox.md) e
   ...
   [contrato de diretórios e flyouts](admin-directory-flyout-contracts.md)
   e consultar `pattern.admin-directory`, `pattern.flyout-actions` e
   `pattern.interaction-states`. Para qualquer card administrativo, Instituições é a baseline automática e deve ser
   ...
   [contrato de tabs lineares de diretório](directory-linear-tabs-contract.md)
   ...
   [contrato de formulários](form-layout-contracts.md), consultar
   ...
   [contrato de páginas de erro](error-page-contracts.md). Não
   ...
   [runbook semanal](weekly-superadmin-ui-review.md).
...
3. Abrir somente arquivos e documentos apontados pelos resultados relevantes.
   ...
   [matriz de evidência dos estados interativos](interactive-state-evidence-matrix.md)
   e preencher mentalmente, para cada estado pedido ou alcançável no controle:
   implementação real,
   componente/contrato, teste comportamental e golden exato, indicando lacunas. Não limitar o inventário ao verbo do pedido: introduzir um
   ...
   não existir, registrar a lacuna e verificar pelo contrato aprovado no recorte
   autorizado. Um golden novo não comprova aprovação por si só. Pedir decisão
   quando faltar a definição visual, antes de oficializar um padrão novo.
4. Reutilizar ou compor tokens, componentes e padrões antes de propor algo novo.
   ...
   O card de Instituições define a anatomia dos cards administrativos.
   ...
   anatomia sem receber semântica interativa. Nessa família,
   ...
   Admin/Superadmin, usar `CoeloAdminExpandableStatusIndicator`. O indicador começa sem texto e
...
5. Respeitar [fronteiras de pacote](package-boundaries.md).
6. Se nada atender, seguir o
   [contrato de proposta](component-proposal.md) e aguardar aprovação.
   Para captura isolada de hora, consultar também a
   [contrato do seletor de hora Coelo](coelo-time-picker.md); somente
   consumi-lo quando seu frontmatter estiver `approved` ou `implemented`.
7. Registrar o padrão aprovado antes do código.
8. Implementar; atualizar índice, catálogo, exemplos e testes.
9. Executar a [verificação proporcional](verification.md) e relatar
...
Uma autorização já dada para a mudança permanece válida; não pedir novamente.
Correção para cumprir contrato aprovado segue o recorte autorizado.
