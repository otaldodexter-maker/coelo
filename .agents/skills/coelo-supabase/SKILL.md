...
- `list_visible_happens_feed` (Acontece) é um invólucro: contexto em `list_visible_happens_feed_context_v1` (sem grant a cliente) ∪ `official_posts` publicados (lote 107). Mudança no feed de contexto vai na `_context_v1`; o cursor (at, kind, id) tem de valer nas duas.
